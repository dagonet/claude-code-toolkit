...
12 hook scripts (2 lib files), 8 skills; 212 → 230 consistency assertions and 204 → 257 hook fixtures. The python3-only and jq-only cases SKIP where that interpreter is absent, so the fixture total is host-dependent: 257 is the count with node, python3 and jq all present.
