...
**Process note: branch `v4/1.1` (cut from `main` at `038f247`, the v4.1.0 tag); Tasks 1, 3 and 4 ran in parallel on disjoint files (server / consistency script / hooks), Task 2 followed Task 1 on the server, Task 5 (skill + docs) followed Task 2, Task 6 is this release commit.** Hook suite 1182 → 1213 assertions (Task 4; the implementer's own GREEN run was lost to a controller duplicate-run kill mid-task, so the confirming GREEN — 1213 passed / 0 failed — was measured by the controller directly from the implementer's worktree at `13e9052`, labelled as such). Server suite 441 → 457 (Task 1, across an initial pass plus a fix round) → 468 (Task 2, ten new test functions plus one new parametrize case). Consistency 420 → 422 (Task 3: check 55 plus check 42 arm 2's own counted PASS/FAIL/SKIP line — the design doc's own arithmetic was one short for missing the second of those) → **423** (this commit's check 56; the design doc's §9 total is corrected here to say so, with the reason). Verify `LINES` 30 → 31 (Task 2, `project_md_seed_differs`). Bootstrap fixtures unchanged at 85 assertions — no task touched `scripts/test-setup-project.sh` or its fixtures this release.
...
### Fixed
- **`migrate_v3_to_v4` derives every carried-forward entry instead of reconstructing it — this release's silent enforcement-layer warning (#17 ×3 symptoms, #20).** The old loop held each v3 manifest entry and discarded it: (a) it re-rendered every template-owned entry from the checkout's CURRENT template and stored that hash for files it never wrote, so a v4.0.3-held consumer migrating against a v4.1.0+ checkout got `LOCAL_EDITED`/`template_changed: false` on exactly the files the release itself changed, with step 2's model-bump rule staying silent on the one release that changes the enforcement layer; (b) when the template no longer shipped a `deletedAcknowledged` file, the recompute resolved to nothing and stored `"hash": ""`, which `classes_and_hashes` then correctly FAILed with a remedy that could not fix it (measured on MM-Agent); (c) it rebuilt every `once` entry as a bare `{"ownership": "once"}`, silently dropping `reason` and any other consumer annotation, reported in no field (panoscribe). **The release's first principle, stated plainly: a migration transforms entries, it does not rebuild them.** Fix: for every entry the migration does not write, `entry["hash"]` now carries forward unchanged and annotations merge through the existing `carry_unknown_file_keys`; `CLAUDE.md` stays special-cased, since it IS written fresh. Grant agents are the only entries that need a new hash (the v3-stored hash is un-spliced; disk holds held-template + the tools edit): the fix renders the HELD template and splices the grants on it — and **this release's second principle, cannot-determine refuses**: if the held base is unavailable, the migration REFUSES for that grant agent (loud, recoverable, nothing written) rather than store an un-spliced baseline that would read as a silent, permanent `TEMPLATE_UPDATED` forever. The dry-run now asserts its own invariant beside the fix: for every entry it will not write, previewed hash must equal stored hash — measured FALSE on three real consumers before the fix (yutraffic's preview: `66f45cfd`/`048a7245`/`b0f03feb` diverging from its v4.0.3 disk), true by construction after, and a tripwire against any future baseline rewrite, on the consumer's own machine, before a write. A second review round closed two narrower gaps in the same fix: the `CLAUDE.md` branch had returned a bare `{"hash", "ownership"}` while every other branch merged annotations, silently dropping a consumer's `reason` on that one path — it now carries annotations the same as every other branch, only the hash is recomputed. And the two `once`-class SEED entries (`.claude/project-instructions.md`, `.claude/agent-grants.json`) get their own dedicated helper that carries any annotation forward but unconditionally PINS `"ownership": "once"` and drops any `hash` — a plain `setdefault` would have left a pre-existing entry that already read `"ownership": "template"` untouched, shipping a v4 manifest that treats one of those seed paths as template-owned with a stale hash, so the *next* sync would silently overwrite the consumer's own instructions or grants file: a lost ownership guarantee, strictly worse than the annotation-clobber bug the round was closing. Measured: server suite 441 → 452 (initial fix) → 457 (second round, five new tests, two of them the discriminators for the corrected seed-entry fix).
- **The grants detector now routes through the region split (#31).** It classified a whole agent diff as `mixed` whenever an agent carried a `tools:` addition together with region content, so `grants_plan` stayed empty and `.claude/agent-grants.json` was written `{}` — MM-Agent's 16 (Explore) + 22 (architect) project tool grants got no durable home, the exact risk grants exist to close. The detector now runs `_split_custom_region` on both sides first (the same split `region_status` already uses) and tests only the remaining, non-region diff for tools-only shape; the region split, not a second stripper. Fixture: MM-Agent's hand-derived grant plan reproduces exactly, and after writing the grants and re-applying `source="template"`, the agent reads IDENTICAL.
- **`project_md_seed_current` gains a harm-keyed third arm; the v4.1.0 CHANGELOG's own claim about it was false as shipped (#22).** The line keyed on two v4.0.1/v4.0.2 sentences only and never on the R-G reword, so it reported "seed is current" on every consumer whose once-class, unscoped, CLAUDE.md-priority `.claude/rules/project.md` still pointed every session at `PROJECT-CUSTOM`, a region this toolkit deleted from a file `deny-claude-md-writes.sh` now refuses to let anyone edit around — measured on four consumers; last release's "reads INFO older" line was wrong the day it shipped. Fix, and the release's fourth principle stated exactly: **mine differs and mine is harmful never share one line.** The harm-keyed question gets its own arm — header still names `PROJECT-CUSTOM` by name → INFO with a remedy to repoint the sentence at `.claude/project-instructions.md`; the differs-keyed question gets its own new line, `project_md_seed_differs` — reports whether the header still byte-matches the shipped seed as a FACT with NO remedy in three of its four shapes (not present; shipped seed unreadable; prefix holds, own rules appended after it) and a pointer-only remedy in the fourth (prefix fails, the consumer edited the seed text itself) — a consumer who deliberately edited a file they own is never told to discard the edit. `LINES` 30 → 31.
- **The command-prefix gap closes (#11) — the first non-hotfix item, and the silent one.** `gc_script_body` anchored on `$1` after `gc_segments`' quote-strip-and-split, so `command bash x.sh`, `env X=1 bash x.sh`, `exec bash x.sh` and `nohup bash x.sh` never reached the 16 KB body reader that scans a script for a gated verb, while the git-command detector's own token scan already caught the identical wrapper shapes ahead of a bare `git` invocation — the hole needed both a gated verb inside a script AND a wrapped invocation. Fix: the leading run of the segment is now SCANNED instead of anchored — skip `VAR=value`/bare-word tokens, stop at the first `bash|sh|source|.` token, stop at the first token that looks like an operand — deliberately not a `KNOWN_WRAPPERS` enumeration, which would miss `time`, `timeout`, `stdbuf`, `xargs -I{} bash` and whatever arrives next. **The reader moves in both directions, and both are named here, on the record.** Stronger: `/usr/bin/env bash`, `env -i bash`, `nice -n 10 bash`, `timeout -s KILL 5 bash`, and `command`/`env`/`exec`/`nohup` wrappers now resolve to the wrapped script and its body is read. Narrower: `"bash" probe.sh` (a quoted interpreter head) was read at v4.1.0 — under the old `$1`-anchor, `gc_segments` had already stripped its quotes before that check ran — and is NOT read at v4.1.1, since the new scan sees the opening quote on the very first token and stops before ever comparing it; an accepted regression, decided here, not inherited from a fixture label. Two pre-existing residuals, unchanged by this fix, documented rather than assumed closed: `find … -exec bash {} \;` / `xargs -I{} bash {}` reach the real `bash` token but resolve the path placeholder `{}` to a literal, nonexistent file (RESIDUAL, not a new hole); `bash "my script.sh"` (a quoted path containing a space) is RESIDUAL for a separate, pre-existing reason — `gc_segments` deletes quotes before word-splitting, so the extracted path is `my`, not `my script.sh`, unrelated to the quote-stop rule above.
- **`deny-claude-md-writes.sh` gains one path normaliser (#23) and a basename pre-filter (#24).** Measured bypasses, all exiting 0 with the write landing: the MSYS `/x/...` form (never folded to the drive-letter `x:/` root the payload's own `cwd` resolves to), `./CLAUDE.md`, `docs/../CLAUDE.md`, and `claude.md` on a case-insensitive filesystem; a relative path also resolved against the repo root rather than the payload's `cwd`, so `cwd=<root>/docs` + `CLAUDE.md` wrongly denied `docs/CLAUDE.md`, out of the hook's own stated scope. `dcm_norm` now resolves a relative path against `cwd`, folds an MSYS `/…` prefix via `cygpath -m` when available (confirmed to work on a nonexistent path — a pure lexical mount-table rewrite, so the normaliser still never needs the path to exist), falls back to a single-drive-letter fold on a `cygpath`-less host, and case-folds on Windows, applied to both sides of the comparison. A fix round corrected two measured defects in the first cut: an EMPTY read (no `file_path` key at all, or unparseable stdin) fell through the old case statement into an implicit allow — the exact "cannot-determine" case this hook exists to refuse — so an empty read now skips the pre-filter's shortcut and falls straight through to the full parser path, which fails closed exactly as before the pre-filter existed; and the `#23` fixtures' own `cwd` (built under system `mktemp -d`, resolving under Git Bash's own `/tmp` mount — a three-character segment the single-drive-letter fold never matched) went unfolded, so `cygpath -m` is now applied to any input starting with `/`, restricted to that shape so a RELATIVE input is never handed to `cygpath` before being joined against the payload's own `cwd` (`cygpath` on a relative path resolves against the hook PROCESS's own `cwd` — a different, wrong bug). `#24`: measured ~1256 ms per editing call before this change (four interpreter spawns ahead of the old path-mismatch exit, on every Edit/Write/MultiEdit/NotebookEdit); a basename pre-filter (`grep`+`sed`+`tr` only — a plain case-insensitive `*claude.md` suffix check, not a four-shape separator enumeration, which would have left a case-insensitive backslash form, `C:\proj\claude.md`, unmatched) now exits before any parser runs for a non-`CLAUDE.md` payload, measured ~368 ms after — about 3.4× faster on the same machine; `dirname`, needed unconditionally by the hook's own first line to locate `lib/json.sh`, joins the documented minimum PATH tool set beside `sh git sed grep head tr`. `#32` (notes, no code change): the hook is version-gated, so a consumer probing it before migrating to manifest v4 reads exit 0 as inert, not disabled; and the harness validates an Edit's `old_string` before `PreToolUse` runs, so an Edit whose `old_string` does not occur returns "String to replace not found" with no hook error at all — only an edit that would otherwise succeed exercises the hook.
- **The gate's environment fingerprint widens `dist` (#15).** `gc_gate_env` derived `pyvenv`/`dist`/`py` only from `<repo>/server/.venv`, so a consumer with system Python and no in-repo venv read all three `absent` and the fingerprint fell back to the node version alone (penumbra measured `pyvenv=absent|dist=absent|py=absent|node=v24.12.0` on a gate that ran 881 pytest assertions) — item 13's 24 h tree-plus-environment acceptance could not see a python-side dependency change on exactly the repos whose gate IS python. `dist` — the only contributor that actually moves on a dependency change — now widens to the site-packages of whatever interpreter the gate runs, never `py` alone (a bare version string would grant the extension while `dist` stayed `absent`, a regression wearing a fix). **Cannot-determine refuses, a second time this release: the check's own comment states the polarity so nobody "corrects" it — ANY contributor reading `absent` VOIDS the extension** (a sha-exact artifact only); a node-only consumer earns the extension back by adding contributors that do not read `absent`, never by `py` alone reading present.

### Added
- **`manifest_migration` replaces the frozen, v2-only `migration_required` as the honest migration signal (#14, #16).** `migration_required` answered ONLY the v2→v3 question, hard-coded `False` on the v3+ path — measured on both stale and current servers reading `false` beside `template_verify`'s own `status_clean FAIL MIGRATION_REQUIRED` in the same call (panoscribe's is the decisive pair, of five data points); `registered_tools` naming `template_migrate_manifest` is necessary but not sufficient, since v4.0.x servers already registered it. **Read `manifest_migration` — `{"from", "to", "required"}`, computed against the CURRENT template's manifest version — and ignore `migration_required`, which is frozen at its old v2→v3-only value for this one release so existing readers keep working, and is removed in v4.2; the two fields will show different answers on a v3-or-later manifest, and that is by design, not a bug.** Step 0's currency check is restated as the only one that matters: `server_commit` must equal the toolkit checkout's own `git rev-parse HEAD`, never `git rev-parse <tag>` — an annotated tag resolves to the TAG OBJECT, not the commit it points at; use `<tag>^{commit}`.
- **Case 2 now compares this consumer's own tracked paths, not the whole `ownership.json` universe (#9).** `derive_template_version`'s three callers passed `rules.tracked_paths`, which includes paths (`user-level-reference/**`) no consumer manifest actually holds, so a docs-only commit to one of them (#145) flipped every consumer to case 3 (`template_version` null, `untagged_template_tree`) until the next tag. The callers now pass a new `consumer_template_paths(manifest, rules)` — the template-side paths of THIS manifest's template-owned entries, resolved to repo-root git paths (a bare template-relative path would compare the toolkit repo's own root `CLAUDE.md` instead of `templates/<variant>/CLAUDE.md` — confirmed by the first, wrong implementation failing its own fixture) — so a commit touching only paths this consumer does not track keeps case 2 and the tag's version, while a consumer whose manifest DOES hold that path still reads case 3. Consequence, stated once and plainly: `template_version` is written at finalize time as a function of the consumer's own file set at that moment, so two consumers at the same template commit can legitimately record different values, and **a fleet roll-up that compares `template_version` across consumers is comparing values drawn from different universes, not one fact.**
- **Check 37 globs instead of enumerating, and new check 55 pins the declaration it enumerates from (#29).** Check 37 hard-coded the six `templates/<v>/CLAUDE.md` paths inline while check 26 iterated the declared `$VARIANTS`; an undeclared `templates/go/` escaped both while both reported green, and under manifest v4 every consumer bootstrapped on it would get the context-mode plugin's routing block re-appended forever (the sync discards it, the deny hook plus `claude_md_identical` forbid a consumer sentinel). Check 37 now globs `templates/*/CLAUDE.md CLAUDE.md`; new check 55 asserts the declared `$VARIANTS` list equals the set of `templates/*/` directories on disk, compared as SETS — the declaration at line 17 stays as a statement of intent this check verifies reality against, never reordered to satisfy the comparator. **#28 (a dedicated consumer-side context-mode verify line) is closed as unnecessary, not built, now that this lands:** `claude_md_identical` plus a sentinel check that can no longer miss an undeclared variant already give full coverage — a `CLAUDE.md` that diverges from the template, the plugin's routing block included, reads `LOCAL_EDITED`/`CONFLICT` before a dedicated line would add anything.
- **Check 42 gains a non-decreasing floor arm; the newest CHANGELOG section must sign its own judgement (#13).** Check 42 failed only when `requires_skill` read ABOVE this toolkit's own `VERSION`; a floor that stayed stale — too LOW — was invisible to it (measured at v4.1.0: everything but the floor moved, still 420 PASS / 0 FAIL). Arm 2 asserts `requires_skill` is non-decreasing since the previous release: the previous release's NAME comes from the CHANGELOG's own second `## v` heading (the gated commit's own statement, never `git describe --tags --abbrev=0`, which answers "nearest reachable tag" and moves with reachability); the comparand commit is `git rev-parse "v${version}^{commit}"`; the previous floor is read from THAT commit's own `templates/ownership.json`, never from its `Floor reviewed:` line, which is presence-checked and never parsed as data. An absent local tag SKIPs by name (fetch tags to enable the check) rather than failing closed — arm 2 is a DETECTION, and failing closed on a merely-unfetched tag would block the gate for no safety gain, the opposite polarity from #15's extension above, which is a PERMISSION and must fail closed on absent data; the release task's own step names `git fetch --tags` as its own call before the gate, so this SKIP never shows at the moment it matters. A declared-floor-equality arm (arm 3) was considered and NOT built — both releases' floors staying stale together is exactly the green case it would have been built to catch. Instead, **new check 56 asserts that the newest `## v` heading names THIS release's own `VERSION` (heading version equals `VERSION` line 1) and that section carries a signed `Floor reviewed:` line, in one counted PASS/FAIL line** — either extraction (VERSION, the heading) being empty fails outright, checked before the comparison, and the `ko` message names exactly which of the three conditions fired; the judgement itself is a compatibility call a person signs, this release above, not a value a check can derive. (The tie to `VERSION` was added in a fix round after the outside reviewer's read at `0e20e34` enumerated every `## v` heading read in the script — exactly three, none of which compared a heading to `VERSION` — and found that a release bumping `VERSION` without adding its own section would otherwise pass on the section still sitting from the release before it.)
- **The fifth principle this release states: a harm-keyed arm needs a release-task step, not a detector, to ask what the release itself retires.** Answered directly for this release (Task 6 report, step 2): diffing every seed file (`templates/*/.claude/rules/project.md`, `templates/*/.claude/project-instructions.md`) against v4.1.0 finds nothing retired — this release adds a harm-keyed arm and a fact line to `project_md_seed_current`/`project_md_seed_differs` but changes no seed byte.
- **The sync-template skill (marker → v4.1.1) closes five items PR #150's own body claimed but never shipped — the release's third principle, on the record, three separate times:** a claim about what a check does is written from the check, not the ruling that motivated it (the seed-check correction above — R-G's "reads INFO older" line was written from the ruling, not from `verify.py`, and was false as shipped); a PR body is a claim about a diff, written from `git show`, never from a brief or a report (#150's own body claimed the step-3 probe rewrite while that rewrite was still pending, unmerged); a review verdict carries a scope line (an unscoped `CONFIRMED` on Task 4 gated a merge without naming what it covered — corrected by the outside reviewer, who set the convention on themselves for every verdict after). The skill items: #1's step-3 probe rule now teaches the DATA-FILE shape explicitly — payload strings live in data files written with the file tool outside the repo, the script only pipes them and carries no gate verb, and assembling the verb from string fragments is named as evasion, not a loophole (measured `2 0 0` on two real consumers). #3's `cmd_len` check is corrected to `len(invocation) + 1 (gc_augmented_cmd's own separator newline) + len(body)`, with `body` read via `head -c 16384` rather than `wc -c` — `wc -c` counts a script's own trailing newline, which command substitution always strips before `gc_script_body` appends it, over-predicting `cmd_len` by exactly one on any newline-terminated script (measured directly against two probe-file shapes, one newline-terminated, one not — only the fixed recipe matches on both). #7's freshness check now filters to the `last-pass.*.json` glob explicitly, never "newest file in the gate dir" (a stale `last-precommit-noop.*.json` read newest 24 s after a real commit, on one consumer). #19 deletes "a chained `git add && git commit` is fine" — the gate hashes the working tree's TRACKED files via a temp index, so a commit introducing NEW files mismatches its own artifact (panoscribe measured three distinct trees across parent/artifact/commit) — `git add` in its own call, then commit, always. #25 generalises the `shutil.which` warning: a probe that execs by bare name measures whatever PATH resolved, and a UNIFORM result across arms that should differ is the tell that nothing really executed (WSL `bash` gave uniform false allows). §8a/9b baselines are restated as `23/0/1/7` and `24/0/0/7` (v4, two shapes) and `20/0/4/7` / `19/1/4/7` (v3 legacy / v3 window) — one INFO higher than the design doc's own `23/0/1/6`/`24/0/0/6`, written before `project_md_seed_differs` existed; the baselines hold from the server tip that carries it onward. `docs/template-sync.md:309`'s "same two reasons as" → "same two v3 reasons as" (#12), the one-word fix that inoculates the sentence against a reader assuming it covers the v4 reasons too.

### Changed
- **The backlog this release ships was mislabelled "v4.0.4 candidates" in v4.1.0's CHANGELOG.** It is corrected here: the next patch after v4.1.0 sits on 4.1, not 4.0, and shipped as v4.1.1, never as a post-tag commit on `main` (spec and plan lived on branch `v4/1.1` throughout).

### Downstream migration
0. Install after the machine's next reboot, once every session is closed (the live `bash server/install.sh` venv launcher lock is why the deadline exists); restart the MCP server and confirm `template_load_manifest` reports `server_commit` equal to `git -C <templateRepo> rev-parse HEAD` and `manifest_migration` in its response.
1. Run `/sync-template`. The hook, consistency-script and skill changes above arrive with the sync (project-scope hooks reach a consumer at its next `settings.json` apply, same as every prior release); the deny hook's normaliser and pre-filter, and the widened gate fingerprint, are inert until then.
2. **Already migrated under v4.1.0, before this fix? Repair, do not re-migrate** — `template_migrate_manifest` on an already-v4 manifest is an idempotent no-op and will not undo the old bug retroactively. Check `classes_and_hashes` for any `deletedAcknowledged` entry whose hash reads BLANKED (the old `""`-on-missing-template bug) and restore it by hand from `<backup_dir>/template-manifest.json.pre-migration`, the pre-migration copy the migration itself wrote (MM-Agent's own remediation); separately, check every carried-forward entry against that same pre-migration manifest for a dropped `reason` key and re-add any that vanished, by hand.
3. **The once-class seed needs a separate, manual repoint regardless of when you migrated** — this fix does not touch it, because it is `once`-class and no sync ever writes it: check `.claude/rules/project.md`'s header for either drift arm (the pre-v4.0.2 wording, or the newer wording that still names the retired `PROJECT-CUSTOM` marker) and hand-edit whichever is present.
4. A `"Merge Without Review"`-shaped refusal on the MCP merge tool has been observed in exactly one session and not reproduced in two others (tally 1/3) — no discriminator for when it fires is claimed here. If you hit it, the legitimate routes are the user's explicit approval, a `code-reviewer` pass first (prefer this one), or the user adding a permission rule themselves — never route around it via `gh`, which is gated by the identical mechanism (the session cwd's HEAD and the gate artifact, never a parsed command segment) and will not merge unreviewed work either.
5. The whole-region import delivers large content without loss through the ordinary sync path: MM-Agent's post-migration `.claude/project-instructions.md` measured 69,639 B — its region content plus the seed header, delivered whole via the file's own last-line `@` import, no truncation.
6. `docs/template-sync.md:309`'s corrected sentence and the context-surface tables' new v4.1.1 column (unchanged from v4.1.0 — no task this release touched a `CLAUDE.md`, `project.md`, `project-instructions.md` seed, the user-level `CLAUDE.md`, or a `PROJECT_CONTEXT.md`) need no consumer action.

**Known issues, open:** #5 `.ps1 -DbFilename` accept-and-warn; #6 `region.sh --scan` not gitignore-aware; #21 a canonical form for the three region byte counts (v4.2 — the delta is signed and content-dependent, a canonical form on both sides is a byte-semantics change out of proportion for a patch); #11's second half, a named auditable allowance for `bash scripts/test-hooks.sh` under this repo's own hooks (the prefix gap above closes the silent half only).

## v4.1.0 — 2026-09-20

**Process note: spec/plan PR #144 (amended by #146 from an open-brain consumer review), four task PRs (#147 Task 2, #148 Task 1, #149 Task 3, #150 Task 4), Task 5 the release commit.** Hook suite 1162 → 1182 assertions (Task 2); server suite 353 → 441 (Task 1, across the original six commits and two controller-directed fix rounds); consistency 411 → 420 (Task 3); bootstrap fixtures 85 assertions, 80 passed / 5 skipped (`scripts/test-setup-project.sh`, the 5 pre-existing gate-only rows). Rulings R-A through R-S in the plan's ledger; two corrected mid-implementation — R-K (`import_line_present`'s FAIL case, dropped then restored, reviewer Q1) and R-I (amended R-S: the skill marker and floor move with `VERSION` in the release commit, not with the skill body, Task 4).

### Added
- **Option B lands: `CLAUDE.md` becomes template-owned and byte-identical per variant; `.claude/project-instructions.md` (once-class, imported via the file's own last line, `@.claude/project-instructions.md`) carries what used to live in the PROJECT-CUSTOM region.** Five files, one ownership table (spec §1): `CLAUDE.md` (template — overwritten every sync; any consumer diff outside the import line is CONFLICT and the sync refuses to apply, never "keep mine"), `.claude/project-instructions.md` (once — project-owned, seeded empty at bootstrap), `.claude/agent-grants.json` (once — project-owned, seeded `{"schema":1,"grants":{}}`), `.claude/agents/*.md` (template — stored hash is now the SPLICED content, template + grants), `.claude/rules/project.md` (once, unchanged). The region stays live in the other 47 regioned files (6 `AGENT_TEAM.md` + 41 agents, v2.1.2) — retiring those is a v4.2 decision (R-H).
- The template tail (all six variants, `wc -c` measured): out = 192 B region block + 151 B lines 3–4 blockquote + 1 B blank line + 124 B old line-123 comment = 468 B; in = 320 B tail block + 132 B rewritten line 123 (keeps the `context-mode` sentinel check 37 pins, R-F) = 452 B; net **−16 B**. Task 3 additionally dropped `, `CLAUDE.md`` from the template's own Write-surface enumeration (line 32, one of R-L's deferred sites) for **−13 B** more, so every variant's measured net is **−29 B**: general 6143→6114, dotnet 6140→6111, dotnet-maui 6139→6110, rust-tauri 6141→6112, java 6135→6106, python 6139→6110 — 30–38 B headroom against `BUDGET_CLAUDE_MD` 6144.
- **Agent tool grants** (`.claude/agent-grants.json`, spliced by the server into the shipped agent's own `tools:` line — `template_content()` is now the ONE producer of spliced content, all ten call sites converge on it): a grant naming a token in `UNGRANTABLE_TOOLS` (a hand-written POLICY set, not a derived one, R-Q — every registered `template_*` sync tool, the four merge/PR tokens, and `Agent`; 15 members), or naming an agent with no `tools:` line to extend, is refused by name (`GrantRefused`/`GrantsError` → `{"error": ...}`, nothing written). R-Q also dropped a vacuous test that compared `UNGRANTABLE_TOOLS` against a second, independent hand-written copy of itself — it could only ever catch a typo in one of the two copies. R-E confirmed, measured 2026-09-19: every shipped agent ships exactly one one-line `tools:` allowlist, so the refusal is a real gate against a future regression, not dead code today.
- **`hooks/deny-claude-md-writes.sh`** (project scope only — registered in all six variants' `settings.json`, never mirrored to `user-level-reference/hooks/`, `HOOKS_NO_MIRROR`): denies `Edit`/`Write`/`MultiEdit`/`NotebookEdit` on a manifest-v4 consumer's `CLAUDE.md`; cannot-determine refuses (unparsable stdin, an unreadable manifest — exit 2); allows on v3, no-manifest, or any other path; survives `bypassPermissions`. **Non-goal, stated plainly:** the matcher covers editing tools only — a Bash write (`sed -i`, `>>`, a heredoc) bypasses it; this is a guardrail, not a boundary, and the next sync's `claude_md_identical` FAIL plus overwrite is the actual backstop.
- **Manifest v4** (`instructions_file`, `agent_grants` declaration keys; `requires_server ">=4.1.0"`) and the **v3 window**: detected against the CURRENT toolkit checkout's template, never the consumer's held commit (R-J) — a v4.0.x-era ("v3 legacy") consumer whose held template still carries the region reads exactly as it does today; a v4.1+-era ("v3 window") consumer whose template has lost the region gets exactly ONE fact, ONE FAIL: `CLAUDE.md` reports `MIGRATION_REQUIRED`, owned alone by `status_clean` with the migrate-first remedy (R-K) — the extra FAILs a naive per-line reading would produce are deliberately not emitted, since hand-editing around them (adding the `@` line by hand, hand-creating `project-instructions.md`) would corrupt the migration.
- **Six new `template_verify` lines** after `region_markers` (`LINES` 24 → 30): `manifest_version_supported` (**renamed** from `manifest_version_3`, R-A — a line that PASSED on 4 while still named "3" would lie; `measured` names the version found, `expected` "3 or 4"), `claude_md_identical`, `import_line_present` (v4-only — catches the case where the TEMPLATE loses the `@` line and a consumer syncing from a branch, fork or unreleased checkout matches it, so `claude_md_identical` alone would PASS while the import silently vanished; restored after an earlier ruling dropped it, R-K amended, reviewer Q1), `instructions_file_present`, `agent_grants_resolvable`, `agent_grants_names_known`, `agent_grants_extendable`. Three exact per-situation SKIP/FAIL/INFO sets (v4 / v3 legacy / v3 window) are in the skill's new §8a. `agent_grants_resolvable` has **three** SKIP reason shapes, not two: `"alias X: no census route"` (including its parenthesised variants — no registration, source dir missing, `templateRepo` unknown), `"alias X: census budget exhausted before it was reached"` (a per-call **30s total** census budget, `AGENT_GRANTS_CENSUS_BUDGET_S`, so one hanging/broken server cannot blind the rest of the line — the rest SKIP, never PASS), and — the only actionable one — the alias's own census route existing but FAILING (a broken venv, say), whose failure text replaces `"no census route"` in place. `region_markers` itself is **unchanged** for the 47 files that keep their region (R-H).
- Two bootstrap bugs, found and fixed by Task 3 while landing the template edits: **`--wrap-existing-claude-md`/`-WrapExistingClaudeMd` silently discarded a consumer's existing `CLAUDE.md` content** — it spliced into a `PROJECT-CUSTOM:BEGIN` marker that no longer exists in the rendered template, found nothing, and returned the template unchanged. Now overwrites `CLAUDE.md` with the plain rendered template and moves the pre-existing file's full content into the freshly-seeded `.claude/project-instructions.md` (guarded so a second wrap never re-migrates). **The sh-side generic write path's trailing-newline restoration only ever restored one newline**, silently collapsing the new seed's spec-required trailing blank line to one — fixed with a `trailing_newline_count()` helper that restores exactly as many newlines as the source has.

### Fixed
- `verify.py`'s `run()` was not defended against `compute_status_v3` returning `{"error": ...}` on a malformed `.claude/agent-grants.json` — every `status[...]` read site (eight, one per emitted line) now goes through a single `status_error` guard; a malformed grants file FAILs gracefully (naming "not valid JSON") instead of raising `KeyError` inside `template_verify`.
- The grant-token shape regex rejected every `mcp__MCP_DOCKER__...`-family token outright (`^mcp__[a-z0-9_-]+__[a-z0-9_]+$` never matches an uppercase alias) — widened to `^mcp__[A-Za-z0-9_-]+__[a-z0-9_]+$` (matching check 50's own `TOKEN_RE`), so the `UNGRANTABLE_TOOLS` refusal now fires by TOKEN membership for those three entries rather than by the shape check refusing first.

### Changed
- **R-S (amended R-I): the SKILL.md marker, `templates/ownership.json`'s `requires_skill` floor, and this toolkit's own `VERSION` can only move together, in the release commit.** The consistency script checks the marker and the floor against root `VERSION` (the marker check, check 42, check 44's server/root `VERSION` byte-identity, check 38's new-release-summary-line check), so a partial move leaves the tree unable to verify itself — measured: marker + floor moved alone → **418 PASS / 2 FAIL** (the marker check; check 42); both `VERSION` files moved alone, without the marker or floor (root line 1 bumped, line 2 left as the old summary, the server copy left untouched) → **417 PASS / 3 FAIL** (the marker check fires from this side too; check 38; check 44); the full move together → **420 PASS / 0 FAIL**. **The floor is guarded one way only.** `check 42` fails only when `requires_skill` is ABOVE this toolkit's `VERSION`; a floor that stayed stale (too LOW) is invisible to it — measured directly, with `VERSION` (both files) and the marker all moved to `4.1.0`/`v4.1.0` but `requires_skill` deliberately left at the old `>=v3.1.3`: still **420 PASS / 0 FAIL**, `check 42`'s own line reading `requires_skill '>=v3.1.3' is tag-shaped and <= v4.1.0`. Moving the floor with the marker and `VERSION` is a release-commit discipline, not something a check enforces both ways; a stronger check — non-decreasing since the previous release tag, and equal to the floor this release declares elsewhere — is v4.0.4 candidate #13, open.
- Test counts, measured: hooks 1162 → 1182 assertions; server 353 → 441; consistency 411 → 420 (four new checks: 51 tail block + last line, 52 no `PROJECT-CUSTOM` marker survives in any variant `CLAUDE.md`, 53 agent `tools:` shape [census + 4 flips], 54 seeds byte-identical across variants); bootstrap fixtures 85 assertions, 80 passed / 5 skipped.
- The plan file's ruling R-I and its Task 4 text (`docs/plans/2026-09-19-v4.1-implementation.md`) are corrected in this commit to say what actually happened: marker + floor + `VERSION` move together in the release commit, not with the skill body.
- **The context-surface tables in `README.md` and `docs/architecture.md` gain their v4.1.0 column** (root `CLAUDE.md` Invariants rule) — measured `wc -c` at this release, both subtotals, per-variant figures, each column's file set named beside it. **What moved, and why the totals GROW for the first time:** the `CLAUDE.md` tail block (Task 3's −29 B trim: −16 B swapping the region/blockquote for the tail block, −13 B dropping `CLAUDE.md` from the template's own Write-surface line) and `.claude/rules/project.md`'s R-G reword (+35 B, byte-identical across all six variants) are both smaller than the new always-imported `.claude/project-instructions.md` seed (823 B, a new row in the injected set — the byte count is the SEED's, a consumer's own content is added at bootstrap/migration and is theirs, never counted here). Net: general's harness-injected subtotal goes 12,303 → 13,132 B and its end-of-bootstrap subtotal 15,725 → 16,554 B, **+829 B either way**. `PROJECT_CONTEXT.md` is unchanged this release (3,422 B, general). The historical "Baseline" and "v3.1" column labels are now "Baseline (v1.3)" and "v3.1 (v3.1.0)" (v4.0.4 item #8).

### Downstream migration
0. Restart the MCP server; `template_load_manifest` must report `server_commit` equal to `git -C <templateRepo> rev-parse HEAD`, and `registered_tools` must include `template_migrate_manifest`. **User-level enforcement moves at release** (the skill floor and marker, once `user-level-reference/` is propagated to `~/.claude/`); **project hooks move at your sync** (`deny-claude-md-writes.sh` is project-scope only — it reaches a consumer's repo the next time `/sync-template` applies `settings.json`, not before).
1. A v2 consumer migrates TWICE: v2→v3, then v3→v4 (both dry-run first, the second following the existing v2→v3 pattern). The v3→v4 dry-run's `template_migrate_manifest` lists (a) the region body that moves into `.claude/project-instructions.md` (an EMPTY region, via `is_empty()`, migrates to a header-only seed), (b) LOCAL_EDITED agents whose diff is confined to `tools:` additions — these become a grants entry (only agents that ship a `tools:` line to begin with, R-D), (c) every other `CLAUDE.md` diff — REFUSED with the diff, nothing written; move the text into the region BEFORE migrating. `backup_dir` is required for a real run; the report lands under it, never in-repo.
2. While the manifest is v3, the server refuses to apply `CLAUDE.md` — sync everything else, migrate, then `CLAUDE.md` applies under v4.
3. Restart the session (session-start delivery) before trusting the new `.claude/project-instructions.md` content.
4. Fixture roles, for anyone reproducing this locally (measured 2026-09-19): open-brain is the EMPTY-region path (a 72 B seed body, header-only after migration, byte-identical `CLAUDE.md` afterwards); MM-Agent is the real-content proof (43,707 B of region content moved without loss). Both syncs are their own users' triggers, not automatic.
5. Expected `template_verify` lines, `post_commit` (30 lines total): v4 skips `{server_skew}` alone; v3 legacy and v3 window both skip `{server_skew, claude_md_identical, import_line_present, instructions_file_present}`, distinguished only by their reason strings and by whether `status_clean` FAILs (window: yes, alone; legacy: no). `pre_commit` adds `tree_clean` to every row's skips.
6. A consumer syncing from a `main` that carries docs-only commits past the release tag reads a case with an honest **null** `template_version` — expected, not a defect (v4.0.4 candidate #9, open); said here so nobody chases it as a bug.

**Known issues carried forward, not shipped in v4.1.0 (v4.0.4 candidates, open):** #5 `.ps1 -DbFilename` accept-and-warn; #6 `region.sh --scan` not gitignore-aware; #7 the #8-fixture's newer-noop file; #9 case-2 on tracked paths (see step 6 above); #11 the canonical `bash scripts/test-hooks.sh` refused under this repo's own hooks (ruling: the refusal stays — it is a real, reproducible self-reference gap — the prefix gap closes first, a named allowance later); #12 `docs/template-sync.md:309`'s one-word inoculation (`import_line_present`'s "same two reasons as `claude_md_identical`" → "same two **v3** reasons"), not applied this release since the file was not otherwise touched in Task 5; #13 the `requires_skill` floor check described above.
...
### Fixed
- `template_verify` FAILed a correct sync for any consumer who acknowledged a kept `TEMPLATE_DELETED` file by hand, per `SKILL.md` step 6 — the server never implemented the `deletedAcknowledged` contract the skill promised (`grep deletedAcknowledged server/src` was empty). `deletedAcknowledged` (`list[str]`, sorted, deduplicated, LF-written) joins `KNOWN_TOP_LEVEL_V3`; `compute_status_v3` now reports a path whose template file is gone AND which is listed there under its own status, **`ACKNOWLEDGED_KEPT`** (summary counter `acknowledged_kept`, both template- and once-class; excluded from `deleted`); `template_finalize_sync` gains `acknowledged_deleted: list[str] = []`, merged into the key (union, sorted) and refusing BY NAME a path that is not currently `TEMPLATE_DELETED`/`ACKNOWLEDGED_KEPT`, or that also appears in `deleted_files`. `unknown_keys_empty` no longer names `deletedAcknowledged`; `classes_and_hashes`'s denominator now excludes acknowledged-kept entries (`identical + acknowledged_kept == template_class_count`), so the line PASSes on a correctly acknowledged sync instead of staying permanently red. The three tests that used `deletedAcknowledged` itself as the preserved-unknown-key exemplar now use the reserved **`x-consumer-note`** shape instead — a plausible feature name getting promoted to known cost this rewrite twice (`lastSynced*` in v4.0.1, `deletedAcknowledged` here), so the new exemplar is a shape reserved never to be promoted (item 15).
- A server process started before a release captured `SERVER_COMMIT`/`__version__` and its ten-tool registry at import, while `v3` and `verify` — and therefore `CAPABILITIES` — were imported LAZILY inside each tool body, so a pre-release server could advertise a capability (`template_verify`) its own registry never gained a matching tool for (v4.0.1 rollout, R29). `mcp.py` now imports `v3` and `verify` eagerly at the BOTTOM of the module — both do `from . import mcp as core` at module level, so a top-of-file placement would create an import cycle that only resolves today by accident — and the TEN lazy import sites inside tool bodies (eight `v3`-facing, two `verify`-facing, one of the ten inside `_cli_verify`) are deleted. New `v3.TOOL_CAPABILITIES = ("template_verify",)` names the dispatch-checkable subset of `CAPABILITIES`; `mcp._registered_tool_names()` reads the live FastMCP registry (`mcp._tool_manager.list_tools()` — the public `list_tools()` is a coroutine, unusable from this sync accessor called inside dict literals with no running-loop-safe `asyncio.run` available there; the choice is documented in the accessor's own docstring) and is now exposed as `registered_tools` on all FOUR `template_load_manifest` response shapes (manifest-None, errors, v3, legacy v2). A fresh-interpreter import witness (a bare `python -c` in a subprocess) pins that both modules are loaded and the registry holds ten names at that same moment; clause 2 of the capability test (`(CAPABILITIES − TOOL_CAPABILITIES) ∩ registered_tool_names == ∅`) fails red the moment a tool-shaped capability is added without a matching `TOOL_CAPABILITIES` entry (item 5).
- `hooks/post-edit-build.sh` matched `case "$raw" in ""|none)` without lowercasing first, so `**Post-edit build**: None` EXECUTED the literal command `None` instead of being treated as unset (`pre-commit-test.sh` already lowercased the equivalent field). The value is now lowercased before the `case`, mirrored in `user-level-reference/hooks/post-edit-build.sh` (item 2).
- `hooks/require-skills-block.sh` read `tool_input.subagent_type`; a payload in another shape yielded an empty value that fell through the `case` as "not a bound type", so a hook-contract change would have silently unbound every coder (fails open). The hook now guards on `tool_name` (only `Agent`/`Task`; anything else, including an agent-adjacent tool like `SendMessage`, exits 0 untouched) and treats `tool_input.prompt` — present on every real Agent call — as the shape witness: absent or empty exits 2, fail closed, naming the payload's keys at EVERY depth (labelled **"Keys present (any depth):"**, not "Top-level keys:", since the list comes from a whole-payload grep and the old label misdescribed it — review finding F1). The pre-v3.0.0 top-level `subagent_type` fallback, dead code once the shape witness exists, is deleted; the FLAT `mkspawn` rows (a shape this toolkit's own hook wrongly read before v3.0.0, never a real client payload) flip from exit 0 to exit 2, with the provenance recorded in the fixture (item 14).
- Four sync-skill contract corrections, each a live defect a consumer hit. Step 1b's worked example promised `template_commit`/`template_version` land as the TAG commit — since v4.0 finalize records `template_commit` as the checkout HEAD you synced from and `template_version` as the nearest tag whose tracked tree is identical, and the example now says so (item 6). Step 2b's two-method backup cross-check compared RESULTS only; one method died on an MSYS path handed to python, returned an empty set, and "both methods agree" printed on a crash — each method now asserts it RAN (exit code, non-empty input) before the comparison (item 9). Step 1 named no branch to sync on; a long-lived session branch carrying the current `CLAUDE.md` region now gets a paragraph saying to sync there, since "sync from clean main" inverts the intent on such a branch (item 18). Step 6b's open question on "keep mine" for a differing untracked hook is resolved: RENAME the project's version to a non-template name, repoint `settings.json`, then apply the template's file — hooks have no PROJECT-CUSTOM region and are template-owned under their template name, so there is no other way to keep one; the route is stated as CONSUMER-side only (item 1).
- `template_finalize_sync(new_files=…)` only ADDS; an already-tracked path updated outside `template_apply_file` kept its stale stored hash and read `LOCAL_EDITED` with an EMPTY `local_diff` — indistinguishable from a real local edit without reading the diff. `status_clean`'s FAIL detail now names each such path with its own remedy ("stale stored hash — pass it in `template_finalize_sync(applied_files=…)`"), and the finalize docstring plus `SKILL.md` step 7 each state the distinction in one sentence: `new_files` registers, `applied_files` refreshes (item 16).
- `tree_clean`'s remedy could not tell a consumer whether the named paths were the uncommitted sync itself or unrelated in-flight work; it now says to commit the sync (step 9) first, or commit/stash unrelated paths separately (item 17).
- The two RUN-GATE self-gate test rows snapshotted the WHOLE shared gate directory (`<common git dir>/gate/`, shared by every worktree since v4.0.1 item 17) and asserted it unchanged across a block of throwaway-repo rows — any concurrent worktree writing into that same directory (a peer's commit, another hook's noop probe) fired the assertion on a healthy run, measured 2/1107 within two days of shipping item 17. Replaced with two NAMED-file mtime+size stamps for the only two files a row could write if it wrongly resolved `REPO_TOP` to `$ROOT` — `last-pass.<HEAD-sha>.json` and `last-precommit.<HEAD^{tree}>.json` — so a real resolution bug still moves a stamp while a concurrent worktree's differently-named artifact cannot (R6).

### Added
- New read-only `template_verify` INFO lines: `legacy_gate_dir` (shape-independent — reads only `pp/".gate"`, no manifest/rules/status — and now runs at every early-return site in `verify.run`, not just the full success path; absent → "no legacy `.gate/` directory"; present → names which of the three known artifact files are still there, with a count of other entries left alone, since a `**Log location**` may legitimately point there) and `once_notes_changed` (one `"; "`-joined line even when more than one once-class file's template guidance comments changed, so `len(lines) == len(verify.LINES)` always holds; null case: "no once-class file has changed template notes").
- `project_md_scoped_consistent`: a scoped `.claude/rules/project.md` (a `paths:` frontmatter key) is exempt from `project_md_seed_current`'s sentence checks — the seed sentences are ABOUT being unscoped — but this new line catches the contradiction of a scoped file that still carries one of those sentences verbatim, naming it for deletion. `project_md_seed_current` itself gains the v4.0.2 "predates v4.0.2" arm: a header carrying neither the v4.0.1 stale sentence nor the new v4.0.2 "picked up at the next session start" sentence, and no `paths:` frontmatter, reads INFO with a replace-header remedy instead of falling through as current.
- `PROJECT_MD_SEED_BODY` (`v3.py`) and `templates/*/.claude/rules/project.md` ×6 gain one new sentence: a rules file created or edited mid-session is picked up at the NEXT session start, not the current one (measured at the subagent boundary — a rules file that only shows in a git-status snapshot is not delivered to a subagent spawned later in the same session). A new consistency check pins `.claude/rules/project.md` byte-identical across all six variants, matching the existing `AGENT_TEAM.md`/`settings.json`/`ops.md` checks.
- `setup-project.sh`/`.ps1`: `LOG_PATH` now defaults to `none` at bootstrap instead of leaving the literal `{{LOG_PATH}}` token; `templates/ownership.json`'s `optional_keys` gains a `none_meaning` ("no log directory declared") for BOTH spellings the six variants use — `Log location` and, on python/java, `Log Path` — no template key rename.
- The reserved `x-*` unknown-key shape: any top-level manifest key starting `x-` is consumer-owned, documented beside `KNOWN_TOP_LEVEL_V3` — never known, never dropped, never reported beyond `unknown_keys`; `unknown_keys_empty`'s remedy for a genuinely unknown key now reads "remove or rename the key (a key starting `x-` is consumer-owned and is never promoted; every other unknown key is preserved but read by nothing)".
- `scripts/test-hooks.sh` gains the item-8 parent-sha artifact row pair: on a Gate-only repo the commit-minted artifact is keyed on the PARENT sha, so `gate-before-merge.sh`'s tier-1 lookup never hits and its tier-2 newest-first tree scan is load-bearing — pinned positive (parent-sha artifact, HEAD's tree → merge allowed) and negative (parent-sha artifact, a foreign tree → blocked).
- New capabilities `deleted_acknowledged` and `registered_tools`, each with a behavioural witness in `test_template_sync_capabilities.py`'s `WITNESSES`; `EXPECTED` is the v4.0.1 set plus these two names.

### Changed
- `scripts/verify-consumers.sh` and `docs/template-sync.md`'s fleet section now document that any future "no stale sync branches" check must key on the REMOTE (`git ls-remote --heads`) or squash-tolerant semantics (`git cherry`, tree equality): after a squash merge, `git branch -d` refuses the local sync branch while the remote delete succeeds (item 10).
- `docs/template-sync.md`'s `template_verify` section documents that a consumer-side audit should assert `HEAD == manifest.template_commit` and `manifest.template_version == git describe --tags --abbrev=0 <template_commit>` (with item 6's identical-tree rule), never a hard-coded release literal — one consumer's audit hard-coded `v4.0.0` and went red on its first correct v4.0.1 sync (item 13).
- This CHANGELOG's own v4.0.1 downstream migration step 6 (delete the three legacy gate artifact files) now says to delete them AFTER `git pull --ff-only`, since the pre-sync hook re-writes `.gate/last-precommit-noop.json` once more between `git checkout main` and the pull (item 11).
- **Deferred:** item 3 (the PROJECT-CUSTOM region seed's new-project MCP-server comment line) → v4.0.3 — budget headroom 1–9 bytes and the single-placeholder region classifier: `BUDGET_CLAUDE_MD` headroom across the six variants is 1–9 bytes, too little for the ~190-byte line without a budget decision or a diet, and `region.sh`'s `is_empty()` plus the server's own classifier each recognise exactly one placeholder-comment shape as empty, so a second comment line would reclassify the shipped region as CONTENT.

### Downstream migration
0. Restart the MCP server, then confirm `template_load_manifest` reports `server_commit` equal to `git -C <templateRepo> rev-parse HEAD`; a 4.0.2 server also reports `registered_tools` — that list, not `capabilities`, says whether `template_verify` is callable.
1. Run `/sync-template` with no arguments.
...
**Process note: nine task PRs, every fix re-reviewed by an outside session at its new sha, a fresh gate run at every stacked head and again at the integration tip.** Twelve review rounds (two on the spec/plan PR, ten across the nine task PRs) raised 39 individually numbered findings; rulings from those rounds are numbered continuously, R1 through R26. Item 24 (a version-floor check against `mcp-dev-servers`) was approved, then DROPPED after measurement: an editable install's dist-info freezes its reported version at install time, so a `>=0.4.0` floor reads UNSATISFIED on a correct machine, and 0.4.0 is in any case the release that REMOVED template-sync, making it the wrong floor in kind — check 50 (item 23) answers the real question, "do the tool names my agents declare exist?", without a version number at all.

### Fixed
- Four `$0`/`$1`/`$2`-style positional tokens in the sync skill's shell snippets (`:404-406`'s `probe()` function, three `region.sh` invocations, one quoted example) are textually substituted by the Skill tool whenever the skill is invoked with an argument, corrupting the snippets. Replaced with named variables (`HOOK_NAME`, `CMD`) and path-relative invocations; a "this skill takes no arguments" line added under the version marker. New consistency check 47 greps every skill body for `$[0-9]`/`${N}` (banned unconditionally — no skill has a legitimate reason to write one) and, separately, for `$ARGUMENTS` in a skill that does NOT declare `argument-hint` in its frontmatter (skills that do — `challenge`, `retro-review`, `sprint` — use `$ARGUMENTS` as designed, in prose, and are not flagged); two-sided via a planted-`$1` control (item 1).
- A key the consumer's own variant declares that is absent, held only under a deprecated spelling, or left as an unfilled `{{...}}` was not surfaced as one actionable list — `key_audit.missing_declared_keys` (`[{key, reason: absent|deprecated_spelling|unfilled, template_default}]`, capability `missing_declared_keys`) now collapses all three; `template_default` is always the consumer's own variant's value. An optional key that is simply absent is never duplicated here — it stays in `optional_absent` only (item 2).
- `template_get_diff(".gitignore")` errored with "Template file not found" — the template copy is named `gitignore` (dotless) and only `template_apply_file` resolved the mapping. `_template_file_path`/`_template_git_path` now share one `template_path_for()` mapping, used by `get_diff`, `apply` and the v2 status path alike; both `compute_status_v3` (the dispatch target for v3-manifest consumers) and the v2 status path now also report the new, additive `new_template_files_detail` (capability `new_template_files_detail`) alongside the unchanged `new_template_files` string list (item 3).
- Step 5 of the sync skill offered `template_apply_file(source="skip")` as the default for a present new file, which manifest v3 refuses outright for `once`-class files ("there is no keep-mine class") — the one action step 5 recommended was the one action that could not succeed. Step 5 now branches on ownership class: a present `once`-class file is registered directly via `template_finalize_sync(new_files=[...])` (zero bytes written, never `skip`), and an absent one is applied then registered the same way `template`-class files always were (item 4).
- `apply_file_v3` refuses `source="skip"` for EVERY ownership class under manifest v3, not just `once`-class — but the sync skill's **step 4 (Resolve Conflicts)** still offered "Keep mine" via `source="skip"` for an ordinary `template`-class conflict, and step 5's case-1 branch and step 6b's present-but-untracked-hook rule did the same. Step 4 and step 5 now offer only accept-template or splice (`source="provided"`) for a template-class file, with "keep mine" expressed as moving the content into the file's PROJECT-CUSTOM region (or hand-merging it); the server's refusal message is quoted once so a reader recognises it. Step 6b's rule is left annotated, not fixed — no known-working replacement call exists yet for a present-but-untracked hook under v3 (item 4 follow-up, step 3b).
- The two setup-DERIVED `PROJECT_CONTEXT.md` placeholders (`{{GATE_CHECKED_BRANCHES}}`, `{{POST_EDIT_BUILD}}`) had no answer for a manual bootstrap that skips `setup-project.{sh,ps1}` — both template lines now ship the literal `none` (with an HTML comment naming the placeholder) instead of an unfilled token; `setup-project.sh`'s `add_derived`/`.ps1`'s `Add-Derived` still override it (a dotnet bootstrap still gets the real `dotnet build` command on Post-edit build) via a new line-level rewrite (`set_derived_defaults` / `Set-DerivedDefaults`), since a plain token substitution has nothing left to match once the shipped default is not a `{{...}}` token (item 5).
- `region_bytes` disagreed with `region.sh --body | wc -c` and with the server's own prior `_region_body`-based count — three instruments, three numbers, none of them the raw span. There is now ONE definition (`region_bytes_raw`, capability `region_bytes_raw`): every byte from the BEGIN marker's closing `-->` up to the first byte of the LINE carrying the END marker, no stripping. `region.sh --bytes <path>` prints the same integer, including when the END marker is indented; on the shipped `templates/general/CLAUDE.md` seed both report `74` (item 6).
- `region.sh` was invoked via `bash "$(dirname "$0")/region.sh"`, which never worked in a skill body (`$0` there is the shell, not the file) — every consumer rewrote it by hand. Now invoked as `bash ~/.claude/skills/sync-template/region.sh` everywhere. The `--scan` rule is now outcome-shaped: an empty result reads as "no regions" ONLY when the command exits 0; any non-zero exit (127 missing, 126 not executable, or anything else) is a hard error naming the script and stops the sync, never a silent "no regions" (item 7).
- The v2-era `lastSynced`/`lastSyncedVersion`/`lastSyncedVersionOf` keys duplicated the server-written `template_version`/`template_commit` under manifest v3, so a client-derived copy could only drift. `template_migrate_manifest` and `template_finalize_sync` now drop all three unconditionally (`v3.SUPERSEDED_KEYS`, capability `superseded_keys`) and report what they dropped in `superseded_keys_dropped`; the sync skill's step 7b, which re-wrote the pair on every sync after finalize, is deleted (item 8, R32/R33).
- The sync skill did not warn that a path handed to a `template_*` write tool must be Win32 or repo-relative — an MSYS path (`/g/…`, as this session's own shell shows the project root) is taken literally and the server writes a stray `G:\g\…` tree instead of the real files. A clause now says so in steps 2b and 3; the server-side rejection of a leading `/<letter>/` on write is Task 5 (item 9).
...
- `setup-project.ps1`'s manifest writer (`ConvertTo-Json` under PowerShell 5.1) wrote `template-manifest.json` with CRLF line endings internally and NO trailing newline at all — `setup-project.sh`'s own writer was already correct; only the `.ps1` side carried the defect. Normalized to LF-only with exactly one trailing LF, matching the server's own finalize/migrate writers (item 12) and what `template_verify`'s `manifest_bytes` line (item 22) checks. Content is unchanged; only line endings and the trailing byte are affected (Task 9 pre-review round, item 12 class).
- `hooks/run-gate.sh`'s tree-hash comment (re-measured for this item) already documented the index-based (`add -u` on a copied temp index) guarantee correctly as of the v3.1 release — the "describes it in terms of `add -A` vs `-u`" defect this item named does not exist on this branch. No change made; recorded here rather than silently dropped (item 13′).
- Migration dumped out-of-region `CLAUDE.md` hunks into `.claude/rules/project.md`, an unscoped file this repo's own `zz-load-probe.md` shows loads at EVERY session start, not "delivered to nobody" as the shipped seed claimed. The hunks now go to `<backup_dir>/CLAUDE.md.out-of-region.diff` (reported as `project_md_record`), beside the existing `.pre-migration` copies; `project.md` is seeded with a header-only body that states the delivery fact correctly and points always-on rules at `CLAUDE.md`'s PROJECT-CUSTOM region instead (item 14).
- Documented, in the sync skill and `docs/template-sync.md`, that TWO gate artifacts exist with different shapes (`last-precommit.json`: `tree` only; `last-pass.json`: `sha` + `tree`) and that `tree` is the matching key while `sha` is advisory — when `last-pass.json` is minted by the Gate fallback during a commit, its `sha` is HEAD at hook time, i.e. the commit's PARENT, not the commit itself. `gate-before-merge.sh`'s success path now echoes which arm matched (`matched: tree` / `matched: sha`). Documented `gate_unverified` (verified in `v3.py`'s `collect_gate_refs` before writing anything): constant `true` on any Gate-declaring repo, for every call; nothing in the tool clears it, because the tool never runs the gate it is naming — not commit state, no clearing condition exists (item 15).
- The gate artifact was per-invoking-checkout (`<repo toplevel>/.gate/last-pass.json`), so a gate run in a linked worktree (a template sync's `hooks/run-gate.sh` invocation, or a worktree coder's own gate) was invisible to a merge attempted from a different checkout of the same repo — `gate-before-merge.sh` reported "No gate artifact found" against a genuinely green gate. `run-gate.sh` now writes `<common git dir>/gate/last-pass.<sha>.json` (`gc_gate_dir`, `hooks/lib/git-cmd.sh`; git ≥ 2.31, `--path-format=absolute --git-common-dir` — the SAME directory whether resolved from the main checkout or a linked worktree, falling back to the pre-4.0.1 per-worktree `.gate` on older git); the filename is sha-keyed, not fixed, since the directory is now shared and two worktrees gating concurrently must not clobber each other's artifact. `gate-before-merge.sh` looks up the exact sha first, then scans `last-pass.*.json` newest-first for a `tree` match (a pinned decision: a commit whose own sha was never gated is still blessed when another commit — a different worktree, a cherry-pick, a message-only rebase — gated the identical tree), then falls back to the legacy path with a deprecation `NOTE:` on stderr; the success echo now names which file matched. `run-gate.sh` writes atomically (`.tmp` + `mv`) and prunes artifacts older than 24h on every successful run. `pre-commit-test.sh`'s `last-precommit.json`/`last-precommit-noop.json` move the same way, keyed on the gated tree. `AGENT_TEAM.md` (×6) now scopes "developers own the merge" to feature work and states that a template sync is PO-performed end to end (sync, commit, gate, merge), since the `template_*` tools are in no subagent's `tools:` list and a worktree coder cannot `git -C` into the main checkout to reach the uncommitted sync changes (item 17).
- `key_audit.optional_absent` named which optional keys were absent but not what absence, or a literal `none`, actually does for each — `key_audit.optional_absent_detail` (`[{key, template_default, effect_when_absent, none_meaning}]`, capability `optional_absent_detail`, source: `templates/ownership.json`'s new `optional_keys` map) now states both per key; `docs/template-sync.md` documents the per-key table and the design rule that a new optional key states this deliberately when it is added. Measuring the CURRENT hook behaviour per key (not assumed) found `**Gate-checked branches**: none` and `**Post-edit build**: none` already read as "absent" (no change), and `**Test**: none` already falls through to the Gate — fixed in `hooks/pre-commit-test.sh` since **v3.0.3** (Task 8½), not by this release. Two `templates/*/PROJECT_CONTEXT.md` comment blocks ×6 that still claimed a literal `none` on `**Test**` "blocks every commit" were stale since v3.0.3 and are corrected (item 18).
- The sync skill's step 9 did not say the Gate-fallback-minted artifact expires (`gate-before-merge.sh`'s existing 60-minute/3600s TTL) or that the MCP merge tool has no `--delete-branch` — step 9 now names the TTL with its remedy (re-run `bash hooks/run-gate.sh`) and shows `git push origin --delete <branch>` as its own, separate tool call, quoting the git-guard's chained-form refusal verbatim (items 19, 20).
...
### Added
- `template_verify(project_path, template_repo="", mode="post_commit"|"pre_commit")` — a new, read-only consumer consistency check (capability `template_verify`) that reports 21 lines (`{id, status: PASS|FAIL|SKIP|INFO, measured, expected, remedy}`) plus a `summary` ("N PASS, M FAIL, K SKIP, J INFO") and an overall `ok`. It verifies END STATE — it would have caught the superseded-keys pair, the missing manifest keys, the missing trailing newline and the `.gitignore` new-file residue, as they would have looked the moment a sync finished — and cannot catch a process defect that leaves no trace once the sync completes correctly (item 22). New CLI entry point: `mcp-template-sync-tools --verify <dir> [--template-repo <dir>] [--mode pre_commit|post_commit]`, exit 0 iff `ok`. New fleet script `scripts/verify-consumers.sh <dir>…` runs it (`mode=post_commit`) against a list of consumer checkouts and prints one summary line each; `unknown_keys_empty` FAILs, by design, on every consumer not yet synced on toolkit ≥ 4.0.1, and the remedy text says so. Wired into the sync skill (step 8, `mode="pre_commit"`, before the commit; step 9b, `mode="post_commit"`, right after it — any FAIL means the sync is not complete) and into `setup-project.sh`/`.ps1` as the last bootstrap step (a FAIL is reported, never fatal to the bootstrap). See `docs/template-sync.md`'s new `template_verify` section for the full line table.
- New consistency check 50 asserts every explicit `mcp__<alias>__<tool>` token — named in a `templates/**/agents/*.md` or `user-level-reference/agents/*.md` `tools:` line, OR in `templates/*/.claude/settings.json` / `user-level-reference/settings.json` (`permissions.allow`, a hook `matcher`, or `permissions.deny`; a wildcard grant like `mcp__<alias>__*` has no name to go stale, so it is not a token) — names a tool the alias's OWN mcp-dev-servers module actually exports, via two independent censuses run by new `scripts/lib/list-mcp-tools.py` (system `python`, never `server/.venv`): a static scan of the source tree and, for a registered alias, an import census (`asyncio.run(mcp.list_tools())`) in mcp-dev-servers' own venv. An `allow`/`matcher`/`tools:` entry naming a nonexistent tool is a dead grant or a disarmed gate hook — red, same as disagreement between the two censuses. A `deny` entry naming a nonexistent tool is ALSO red (unless the name is listed in `C50_SPECULATIVE_DENY`, an exception for a deliberately forward-looking deny — empty today): a stale deny fails OPEN, not closed — an upstream rename of the denied tool would silently make a deliberately blocked operation reachable again while a report-only treatment stayed green. **Check 50's first run found two stale git-tools deny entries** (`git_merge`, `git_push_tags` — never tools in mcp-dev-servers; `git_push` already covers tag pushes via its own `tags: bool` parameter) **and they are removed** from the deny list in all six `templates/*/.claude/settings.json` and in `user-level-reference/settings.json`; a stale deny is inert, not protective. Check 22 (v2.1.1, the pre-existing git-tools-deny-list assertion) is updated to the same 5-name list — its own prior comment ("denying a tool the server does not expose is a harmless no-op") is corrected for the same reason; a new header line pins that check 22's list is deliberately hard-coded (the specification of intent) while check 50 verifies the names are real, so the two are never merged into one derived check. An unregistered family alias (`git-tools`, `github-tools`, `dotnet-tools`, `ollama-tools`, `rust-tools`, `python-tools`) is an in-band SKIP for its import census (static census only), and the whole check is a single SKIP naming `MCP_DEV_SERVERS_DIR` when no mcp-dev-servers checkout can be found (env override or derived from a registered family alias's venv path in `~/.claude.json` — never a hardcoded path). Two-sided via 50c (arm A: a scratch copy of one server module with one ALLOW-side tool function renamed is flagged by the same static scanner and comparison the real check uses; arm B: the same for a renamed DENY-side target) and 50d (a scratch settings.json fixture asserts the allow/deny/matcher split, and the wildcard exclusion, are real) (item 23; item 24, a version-floor alternative, was dropped as measured-unworkable — see the Process note above and the design spec).
...
### Downstream migration
0. **Restart your MCP server first**, then confirm `template_load_manifest` reports `server_commit` equal to `git -C <templateRepo> rev-parse HEAD` (`6043ff7` for v4.0.1). A server started before the release captured its identity (`server_version`, `server_commit`) at import while loading the newer modules lazily from the current tree — it advertises v4.0.1 capabilities under a `4.0.0` / `3ed8a16` identity. Compare `server_commit`, never `server_version`, and never the capability list either. Three things answer three different questions: `server_commit` says what this server IS (pinned at import); the registered TOOL LIST says what it can DISPATCH (also fixed at import — a pre-release server advertises `template_verify` in `capabilities` while its registry holds nine tools without it, so a call fails at dispatch, not at the check); `capabilities` says what the lazily LOADED modules support (they follow the current tree and overstate both). A capability is not a promise of dispatch. A restart alone does not make a server current, it pins whatever the checkout held at that instant, so the comparison is the half that decides. Cheap probe: `template_load_manifest` on any directory without a manifest returns `valid: false` and still reports `server_version`, `server_commit`, `server_source` and `capabilities`. This is the sync skill's own step-1 skew condition and its remedy.
1. Run `/sync-template` once with **no arguments** — the Skill tool substitutes `$0`-`$9`/`$ARGUMENTS` textually across the whole body, and any argument corrupts the skill's shell snippets.
2. Expect `superseded_keys_dropped` in the sync's output (the legacy `lastSynced`/`lastSyncedVersion`/`lastSyncedVersionOf` keys are dropped) and a `template-manifest.json` that now ends with exactly one trailing newline.
3. If your project was bootstrapped with `setup-project.ps1` before this release, `template_verify`'s `manifest_bytes` line FAILs once on your first v4.0.1 verify (the old `.ps1` writer left the manifest CRLF and with no trailing newline) — the same `/sync-template` run self-heals it, since `template_finalize_sync` rewrites the manifest LF-only with one trailing newline.
4. Replace the header of `.claude/rules/project.md` with the v4.0.1 seed (or simply add a `paths:` block) — the old seed's "delivered to nobody" claim is false (this repo's own `zz-load-probe.md` shows an unscoped rules file loads at every session start); `template_verify` reports the stale sentence as an INFO line with this remedy until you do.
5. `CLAUDE.local.md` is no longer offered by the template; your existing copy stays and is never touched by a sync. A project bootstrapped fresh under v4.0.1 or later never receives the always-on MCP/session-start guidance that used to ship inline in `CLAUDE.local.md` (registered-server list, git/GitHub MCP-only requirement, Open Brain session-start mandate, trust/verification, failure handling) — `CLAUDE.md`'s PROJECT-CUSTOM region is the place for that content now.
6. Gate artifacts now live at `<common git dir>/gate/last-pass.<sha>.json` (shared by every worktree, inside `.git`) instead of `<repo toplevel>/.gate/last-pass.json`. Remove only the three old artifact FILES by name — `.gate/last-pass.json`, `.gate/last-precommit.json`, `.gate/last-precommit-noop.json` — **never the `.gate/` directory itself**: `setup-project.sh` leaves `**Log Path**` without a default, and a project that pointed it at `.gate/` keeps untracked, unrecoverable logs there (measured on one consumer: 585 KB). Update `hooks/run-gate.sh` and `hooks/gate-before-merge.sh` TOGETHER: a **partial** hook update (only one of the pair synced) blocks merges until both are applied, because an old `gate-before-merge.sh` never looks in the new shared location and a new `run-gate.sh` no longer writes the old one (the legacy path is still read as a one-release fallback, which is what makes the reverse combination — new `gate-before-merge.sh`, old `run-gate.sh` — keep working). Delete the three files AFTER `git pull --ff-only`: between `git checkout main` and the pull the pre-sync hook runs once more and writes `.gate/last-precommit-noop.json` again (safe to delete by name).
7. The sync also removes two stale `git-tools` deny entries (`git_merge`, `git_push_tags` — names that never existed in mcp-dev-servers; `git_push` already covers tag pushes via its own `tags:` parameter) from your `.claude/settings.json`; a stale deny is inert, not protective, so this is a cleanup, not a behaviour change.
8. The sync skill's old step 7b (re-stamping `lastSyncedVersion`/`lastSyncedVersionOf` after every sync) is removed outright; version labels are server-authoritative under v3 now, and the client no longer writes them.
9. Run `bash scripts/verify-consumers.sh <dir>…` across your consumer checkouts to see the fleet's status at a glance. Expect two known pending-sync FAIL lines on any checkout not yet synced on toolkit ≥ 4.0.1: `unknown_keys_empty` (the superseded keys are still present) and, for a `.ps1`-bootstrapped checkout only, `manifest_bytes` (point 3 above) — both self-heal on the next `/sync-template` run and are the release's own "who still needs to sync" signal, not defects. After syncing, `template_verify` is expected to read all-green (`unknown_keys_empty` PASS, `superseded_absent` PASS).

Also: **nothing about hook BEHAVIOUR changes in this release.** `**Test**: none` already meant "not declared, Gate fallback" as of v3.0.3 (before that: run as a shell command, blocking every commit) — this release only corrects two stale `PROJECT_CONTEXT.md` comment blocks that still described the pre-v3.0.3 behaviour, and adds `optional_absent_detail`/`missing_declared_keys` so the skill can report the (unchanged) per-key semantics without re-deriving them. A consumer syncing v4.0.1 sees new manifest/status fields, not a functional change to any hook. Separately: a `template_*` write tool now refuses an MSYS path (`/g/...`) outright rather than silently mis-resolving it — pass a Windows path (`G:\...`) or a repo-relative path instead.
...
**Three consumer steps, and only three.** (1) `bash server/install.sh` (or `server\install.ps1`) in the toolkit checkout — creates `server/.venv`, an **editable** install, and prints the console-script exe path. (2) Point `template-sync-tools` in `~/.claude.json` at that printed path, replacing any prior `mcp-dev-servers` entry — `setup-project.{sh,ps1}` does this automatically for new projects. (3) Restart the MCP server (a session restart also does it). **Never reinstall to fix skew** — the editable install means a later `git pull` advances the server at the next restart with no reinstall needed; reinstalling into a running venv is the one measured way to break it (locked launcher executables, no shim mid-flight). A project synced under either server (the old `mcp-dev-servers` copy or this one) loads on the other — both registrations keep working through the cutover window.

**The splice floor is not raised.** `MIN_SERVER_FOR_V3` stays `"0.3.2"`; new bootstraps still stamp `requires_server: ">=0.3.2"`; `requires_skill` stays `">=v3.1.3"`. Nothing about the manifest contract changed, so raising the floor would be a lie, and a costly one: `raise_floor` is monotonic and automatic on first finalize, so stamping `">=4.0.0"` would make every existing project unable to load on any 0.3.x server after its first sync, before the consumer decided anything. Not raising it keeps the cutover reversible per project.

**`server_version` is the bare `X.Y.Z` from `VERSION` line 1** — never the tag, never a pre-release suffix; `template_version` stamps `v` + the same line. A consistency check enforces the shape (`^[0-9]+\.[0-9]+\.[0-9]+$`), because a version string in the wrong shape makes `requires_server_satisfied` return `False` for every floored consumer at the one call the whole sync is gated on.

**Two new load-response fields close the two ways "one tag" hides real state.** `server_in_template_repo: bool` — computed server-side (never compare `templateRepo`, which is MSYS-style, against `server_source`, which is Windows-style, as strings: that comparison reports "not re-registered" to a consumer who correctly did) — tells the skill whether this response came from the in-repo server or the old `mcp-dev-servers` copy; the skill reads it with `.get(...)`, never `[...]`, because on a pre-4.0 server the key is absent and absent → falsy → "not re-registered" is the correct reading by construction, not a `KeyError`. `server_commit` — the toolkit commit this server process imported at spawn, also `.get`-read, `null`-safe (a non-checkout install genuinely cannot tell, and that is not "differs") — makes visible that templates read from disk advance the moment you `git pull` while the server is frozen at spawn; the skill warns only when the field is present, non-null, and differs from `git -C <templateRepo> rev-parse HEAD`.

**`scripts/test-server.sh` is the third gate command.** The moved suite — 235 tests, now **249** in the server package including the version and consistency tests this release adds — runs from `hooks/run-gate.sh` at merge time, not on the per-commit `**Test**` path. A missing venv fails it loudly (exit 2); it never skips. Gate time is **environment-dependent — state the spread, never one figure**: measured 32 s idle, 73 s with a concurrent gate running, 364 s on a loaded machine, and multiple hours under a saturated box. Checks 43–46 in `verify-template-consistency.sh` cover this release: 43 (`VERSION` shape, bare `X.Y.Z`), with a **43c** delete-the-guard control alongside it — feeding the shape test a tagged string (`v9.9.9`) and asserting it fails, so a shape check that has gone inert reports red instead of green; 44 (package `VERSION` byte-identical to root); 45 (`MIN_SERVER_FOR_V3` stays a `0.3.x` string, not the current version — the same guard-against-a-good-intention shape as check 42); 46 (the gate line names three commands). The hooks suite (`scripts/test-hooks.sh`) stands at **1076 assertions**; the bootstrap fixture (`scripts/test-setup-project.sh`, invoked as check 27) at **60 rows** — 55 exercised on every commit, plus 5 gate-only rows that run only under `RUN_GATE_ACTIVE=1` (2 installer-parity, 3 hostile-path).

**The existing `--mcp-dev-servers-path` flag stays.** `setup-project.sh:539,542` still use it to gate the `dotnet-tools` and `rust-tools` registrations, which stay in `mcp-dev-servers`. The toolkit's own server needs no flag: it is in the same checkout as `setup-project`, so its path is `$SCRIPT_DIR/server/.venv/…`.

**`mcp[cli]>=1.2,<2` is pinned, and here is why.** A fresh venv resolved `mcp` 2.x, which ships `mcp/server/fastmcp.py` as a tombstone raising `ModuleNotFoundError` — every moved module failed collection. `mcp-dev-servers` 0.3.8 carries the same latent hazard (their PR #43 fixes it in 0.3.9).

**Removal sequencing, and the rollback correction (R30).** `mcp-dev-servers` 0.4.0 removes `template_sync_mcp.py` + `template_sync_v3.py` and their 17 test files only after this release is out and at least two consumers have re-registered and synced green on throwaway clones — never a real project, so verification cannot itself close a door. **The rollback target is `mcp-dev-servers` 0.3.9, not 0.3.8.** The spec's item 11 said "0.3.8 stays installable" when written; that was false: 0.3.8 is not fresh-installable from a clean environment — it resolves `mcp` 2.x and fails the same tombstone import above — while 0.3.9 carries the `mcp<2` pin and installs cleanly. Every sentence in this repo claiming 0.3.8 as a working rollback has been corrected to 0.3.9 (`docs/plans/2026-09-12-v4.0-server-move.md` item 11, its implementation plan's Task 13 wording, and `mcp-servers/HOWTO.md`); sentences that only narrate the move's history ("moved from v0.3.8", "tag v0.3.8 = a9d74c4") are left as the historical record they are.

**Reviewer-terms amendment.** Inspect a v4.0 PR via a **detached worktree**, never a branch checkout in the shared `G:/git/claude-code-toolkit` — the outside-review property ("the checker is not the writer") depends on the reviewer never holding a ref the author could also move.

**Process note: fourteen outside reviews across this cutover, every fix re-reviewed at its new sha** — never a green claim taken on the fixer's word. Two rules came out of that round, worth keeping generally: **re-run the original failing case from scratch after a fix, not the probe that found it** — a probe can pass for a reason unrelated to whether the underlying case now works; and **after correcting a figure, grep the tree for the claim, not the reviewer's list of rows** — a claim about installability can be written without its version number attached, so a grep keyed on the reviewer's specific hits misses restatements they never saw.

### Downstream migration

1. **`bash server/install.sh`** (or `server\install.ps1`) in the toolkit checkout — once per checkout, creates `server/.venv`.
2. **Point `template-sync-tools` in `~/.claude.json` at the printed exe path** — `setup-project.{sh,ps1}` does this for new projects automatically.
3. **Restart the MCP server.** Never reinstall for skew — the editable install already picks up a later `git pull` at the next restart.
4. **The editable install's package metadata goes stale on this VERSION bump, and it is not a hypothetical:** measured directly on this release — with `server/.venv` still built against `3.1.6`, `template_sync.__version__` reads the bumped `4.0.0` from the package `VERSION` file (read at import, no fallback) while `importlib.metadata.version("claude-code-toolkit-template-sync")` still reports `3.1.6` (hatch computes that metadata at install time, not at import time) — one test failed, three passed. After `bash server/install.sh` reinstalls, all four pass and the two agree. **After pulling 4.0.0, run `bash server/install.sh` once so the console-script metadata matches** — until you do, `scripts/test-server.sh` is red on this checkout.
5. A project synced under either server (old `mcp-dev-servers` copy or this one) loads on the other; nothing about a sync's outcome changes.
6. **Rollback**, if the in-repo server regresses: `pip install mcp-dev-servers==0.3.9` (not 0.3.8 — see above) and re-register the old exe path.
...
**Why the stance changed — measured, not preferred.** The plugin's routing guidance reaches the model **three times per turn** through its own `SessionStart` and `UserPromptSubmit` hooks, plus a `PreToolUse` tip on every tool call; the block it appends to `CLAUDE.md` is a fourth copy. `ctx_batch_execute`, which that guidance names PRIMARY, failed **28.6 %** of calls across six weeks of transcripts, and **164 dead `ctx_execute` calls** came from subagents that received the guidance but not the tools. It blocks `WebFetch`, `curl` and `wget` to create the need one of its own tools fills. Its sandbox's `/tmp` paths are invisible to `git -C`, so the sync skill already forbade it for this repo's main job. And in an 11-hour session that shipped five releases, it was invoked zero times while being recommended on every call; the one time its post-compaction "session guide" fired, it named the wrong plan file and the wrong mode. v2.0 demoted it to optional on the transcript data; everything since has been defence. `mcp-servers/HOWTO.md` now says so, with the numbers, and says how to remove it. The user-level `CLAUDE.md` and `settings-reference.md` carry the same stance in one line each.

**What stays, deliberately.** Consumers may still run it, and the toolkit cannot control that. So the six template sentinels stay, the `permissions.allow` prefix stays as a no-op, and the sync skill's three defensive paragraphs stay. Nothing shipped ever called a `ctx_*` tool, so nothing had to be removed.

**Step 1c reads two mcp-dev-servers 0.3.5 fields when they are present, and stays correct without them.** 0.3.5 shipped items 1–5 of the agreed split: on that server each `dropped_resolutions` row now carries `ownership` directly, so the step reads `row["ownership"]` and warns only on `template` — on 0.3.4, where the key is absent, it joins against the returned manifest exactly as before, and both paths give the same verdict. And **`dropped_file_keys`** now reports consumer annotations on entries the migration *drops* — the gap this skill named in v3.1.3 as "vanishes with no report anywhere" — partitioning with `unknown_file_keys`; on 0.3.4 its absence is the version signal, not "none", and the 1c-i census remains the only view. `requires_skill` is **still not read** by 0.3.5; the server's own CHANGELOG and its new contract doc (`docs/template-sync-migration-contract.md`) say so in those words. Item 6 stays 0.3.6, gated on the sentinel and arms A/B/C plus the file-absent case.

**Reading that contract doc against step 1c found one error on each side.** Theirs: §2 restates the "two tests, never one — `in` answers the version question" rule the server session had already retracted; reported. Ours: step 1c said `project.md` is "a ~136-byte header and nothing more," generalising a fixture that had `hunk_count: 0`. The contract — and the implementation — write a header **plus the out-of-region hunks fenced as `diff`** when there are any; only the region is never copied. A consumer with real out-of-region edits would have been told their hunks were not written. Corrected, and the sentence saying the docstring "still claims" the region is written verbatim is retired: 0.3.5 fixed that docstring and pinned it against the artifact with a test.

**The injection mechanism, precisely, because "the plugin edits CLAUDE.md" was the question.** The plugin has **two** writers with the same guard: the MCP server bundle's `writeRoutingInstructions()`, and the launcher `start.mjs`, which runs on **every server spawn** — session start and every `/mcp` reconnect. Each appends the 63-line block to `<project>/CLAUDE.md` unless the file already contains the substring `context-mode`, and creates the file if it is absent. That substring is the plugin's only idempotency guard; none of its fifteen `CONTEXT_MODE_*` environment variables disables the write. It is also why `G:\git\CLAUDE.md` appeared on a machine that never wrote one: a session opened with that directory as its project spawned the server there.

**The root `CLAUDE.md` was the undefended file.** Check 37 (v3.1) asserted the sentinel in all six variants and re-measured the guard in the installed bundle — and the toolkit's own root, being "not a seventh variant", was skipped. But the root *is* a project `CLAUDE.md` that Claude Code opens, so it was re-injected on every spawn while every consumer was fine. It now carries the sentinel as an Invariants bullet that explains itself, and **check 37 asserts all seven files** and re-measures **both** writers wherever the plugin is installed, so a change to either goes red rather than silent. With the plugin uninstalled, the predicate arm reports a NOTE and the seven sentinels are still asserted — the check never passes vacuously.

### Downstream migration

1. **Re-copy `user-level-reference/CLAUDE.md` and `user-level-reference/skills/sync-template/SKILL.md` into `~/.claude/`** — one paragraph in the first, two 0.3.5-aware paragraphs in the second. `scripts/verify-user-level-drift.sh` reports 0 when both have landed. **Restart the session** afterwards.
2. **If you run context-mode, nothing breaks.** Your template `CLAUDE.md` already carries the sentinel. If you want to remove the plugin: `claude plugins uninstall context-mode@context-mode`, then restart — a running session keeps the hooks it loaded.
3. Nothing else changed. No template file, no manifest field, no floor.
...
**And the fallback was in a third namespace.** When no version can be determined, both writers used to stamp the string `"unknown"`. The v3 contract names `null` for that field — "the nearest reachable tag whose tracked tree is identical, null when none" — and the sync server emits `null` from the same condition. A sentinel string is **truthy**, so a consumer written against the contract (`if manifest.template_version is None`) sails past the check and then fails resolving `"unknown"` as a ref: the same defect as a missing `v` prefix, one layer down, in the branch nobody exercises. Both writers now emit JSON `null`. `template_commit` keeps `"unknown"`, deliberately — that *is* its contract, and the fixture asserts it.

Two new fixture rows cover that branch, and they are asserted as a JSON **type** rather than by string compare, because `"null"` and `null` are precisely what the rows exist to tell apart. The no-`.git` arm was already a rig for this without anyone intending it: its source copy takes the scripts, templates and reference trees but not `VERSION`, so both writers take the fallback. Verified two-sided — reinstating `"unknown"` in the bash writer turns the sh row red (`want null, got string:"unknown"`) and leaves the ps1 row green, so the two are independently sensitive rather than one check reported twice.
...
**The floor, and why one guard was not enough.** v3.1.0 emitted `requires_server: ">=0.3.0"`. That is too low: 0.3.0 and 0.3.1 read a v3 manifest happily but lack the region splice, so applying `CLAUDE.md` under either **overwrites a populated `PROJECT-CUSTOM` region with the template's empty seed**. Four consumer sessions were measured in that armed state on 2026-09-09 without having installed anything — `migration_required` turned true when *this* repo shipped `templates/ownership.json`, not when their servers changed. `setup-project.{sh,ps1}` now emit `">=0.3.2"`, which `template_load_manifest` enforces on every sync **after** the first, including a downgrade or the same repo opened on a machine whose server process is older. It cannot protect the *first* migration, because a v2 manifest carries no floor at all — so the sync skill covers that moment directly: if `migration_required` is true and `server_version` is below 0.3.2, stop. The skill reads that field from the `template_load_manifest` response and says explicitly not to substitute `pip show`, the dist-info, the tag, or the source checkout — on one real machine all four disagree, and only the load response describes the process that would perform the sync. A running server executes whatever it imported at spawn, so upgrading on disk changes nothing until a restart. The bootstrap fixture's assertion moves with the emitter, so changing one without the other is a red gate rather than a silent mismatch. Agreed with the mcp-dev-servers session, which raised `MIN_SERVER_FOR_V3` to match in 0.3.3.

**Consistency check 39 — the `PROJECT-CUSTOM` markers are load-bearing for DATA, not just delivery.** All six variants must keep both markers and the pointer line. The rationale was previously written as a delivery argument (this is the only always-on channel); the operative reason is worse than that. The markers are how the sync server *finds* a consumer's region: remove them from a variant and the next sync has nothing to splice into, so it discards every consumer's instructions on that variant silently. A check whose stated reason is weaker than its real one invites someone to weigh it wrongly.

**Documentation corrected where it was wrong, not merely stale.** `README.md`, `docs/architecture.md` and `docs/templates.md` each claimed an unscoped `.claude/rules/*.md` "loads at launch at CLAUDE.md cost". It does not — **it is delivered to nobody**, no rules content of any kind is in a subagent's context at spawn even when scoped, and a missing `paths:` key means "matches nothing". The toolkit ships scoped rules only, but for the opposite reason to the one given. Every figure the README asserts was re-measured rather than carried forward, duplicated assertion counts now point at this file so there is one place for them to drift out of, `docs/templates.md` describes the hooks and files v3.1 actually ships, and one field recording a harness's own call was removed from a section where it read as evidence about the library. `docs/verification.md` gains the timing sentence and the `EXP_*_SKIP` derivation.

### Downstream migration

1. **If your server is 0.3.0 or 0.3.1, do not run `/sync-template` — upgrade and restart first.** The skill will now halt a first v2→v3 migration on those versions rather than let it overwrite your `PROJECT-CUSTOM` region. Check what the sync would actually use: `server_version` from the `template_load_manifest` response, not `pip show` and not the source checkout.
2. **A project bootstrapped or re-bootstrapped from this version declares `">=0.3.2"`.** Every sync after the first refuses on an older server, by design. If you keep an older server deliberately, you are choosing a version that cannot preserve your region.
3. **Re-copy `user-level-reference/skills/sync-template/SKILL.md` into `~/.claude/skills/`** — the migration gate does nothing sitting in this repo. `scripts/verify-user-level-drift.sh` reports 0 when it has landed.
4. Nothing in the template tree changed shape; no re-sync is required for this release beyond (3).

## v3.1.0 — 2026-09-09

**Declared keys instead of prose, ownership per file instead of guesswork, and four enforcement fail-opens closed — three of them found by review rather than by the suite. One spec commitment was reversed on measurement and is described here as it shipped, not as it was planned.**

**Phase 0 — template diet.** `CLAUDE.md` ≤ 6 KB and `AGENT_TEAM.md` ≤ 20 KB per variant, enforced by consistency **check 35** as a ratchet: the budget is a measurement of what the files are now, so a file cannot grow back without the check going red and someone deciding it should.

**Phase 1 — ownership foundation.** `templates/ownership.json` (15 rules, first match wins) gives every template file one of three classes — `template` (the toolkit's, overwritten on sync), `once` (seeded then yours forever), and project-by-absence (a file the table does not name is yours, which is why an incomplete manifest is dangerous — see the rerun fix below). `scripts/lib/ownership-classify.mjs` emits `<rel>\t<class>\t<target>\t<rule-index>`; the rule index is exported deliberately so a second implementation can be compared against this one on *precedence*, not merely on verdicts. mcp-dev-servers did exactly that across all 43 template-tree paths and found 0 mismatches including the two cases most likely to diverge (`hooks/lib/git-cmd.sh` matching a `hooks/**` pattern across a slash, and `gitignore` → `.gitignore` at rule 11, the only rename). New **checks 36/37/38**: ownership coverage with both arms (a file with no class is red, and a rule matching nothing is red); the context-mode sentinel; and VERSION's two-line convention, because v3.0.4's first squash bumped line 1 and left v3.0.3's summary on line 2. `setup-project.ps1` writes UTF-8 **without** a BOM; open-brain measured the fix two-sided (31/31 files BOM'd before, 0/31 after, with a red control) and corrected our own claim about it: the `.gitignore` self-heal fires only on the append path, so a BOM'd complete `.gitignore` survives even `-Force`. That is stated with its condition everywhere it appears rather than as a blanket claim.

**Phase 2 — hooks bundle (ENFORCEMENT-CHANGING).** Three new declared `PROJECT_CONTEXT.md` keys read by hooks rather than by agents remembering: `**Post-edit build**` (a per-project PostToolUse build, never blocking), `**PO write surface**` (extends where the PO may write, root-anchored and normalised — `src/docs/x.md` is not under `docs/`, and neither is `docs/../src/x.py`), and `**Gate-checked branches**` (a long-lived session branch may require a fresh gate artifact on merge without being protected). SubagentStop eligibility now comes from `pipeline: true|notify` in the project's own agent files rather than a `settings.json` matcher — `true` means echo plus contract, `notify` means echo only, which exists because one boolean would have put a blocking contract on an agent with no Bash and stalled it permanently. Both gate hashers stage tracked files only, so an untracked file present at gate time no longer enters the artifact tree; the migration consequence is the sentence penumbra's measurement produced: **a `git add` and the `git commit` that follows must be separate tool calls, because the commit hook mints from the index as it stands when the call is submitted.** Refusals now name the clause that decided — the pipe stage, or the earlier checkout that has not run yet — and fd duplications (`2>&1`) are inert while file redirections still classify as movers.

**Four enforcement defects, found by review and consumer measurement rather than by the suite:**

1. **A gate-checked branch glob was word-split unquoted**, so a declared `*` or `m*-*` was pathname-expanded against the *invoking process's* working directory and became filenames — an eligible branch went ungated depending on a directory unrelated to the repository. Measured two-sided: a populated cwd gave exit 0, an empty one exit 2. Two consumers reproduced it on independently built rigs, one of which shares no code with ours.
2. **A merge landing on a gate-checked branch via a compound `checkout && merge` was allowed**, because the move verdict only distinguishes a move onto a *protected* branch, so the check ran against the pre-checkout branch. Routed to the cannot-determine verdict — deliberately not to the gate-checked one, which would have compared the artifact against the wrong branch's HEAD and, with a fresh artifact there, returned a green receipt for a merge it never examined. The suite carries the row that discriminates those two outcomes.
3. **Every git gate returned 0 on a lib that was present but corrupt.** A *missing* `hooks/lib/git-cmd.sh` was already fail-closed via the 127 wrapper; an empty or syntax-broken one was fail-open and silent, and three mechanisms in this project's history produce exactly that state. All four hooks now check that a long-standing lib function is defined and refuse when it is not (`post-edit-build.sh` still exits 0 — PostToolUse must never block — but reports it), all naming the same diagnostic so one grep finds them.
4. **A failing post-edit build was indistinguishable from a passing one** — non-zero exit, hook rc 0, no output at all. Still never blocks; now says so.

**Phase 3 — ownership cutover, partial and honestly so.** `setup-project.{sh,ps1}` write **manifest v3**: `manifest_version: 3`, `variant`, `templateRepo`, `placeholders`, a v-prefixed `template_version`, `requires_server: ">=0.3.0"`, and per-file `ownership` with a `sha256:` hash on the `template` class and none on `once`. The three "extra" fields are load-bearing rather than ceremony: without `placeholders` every template hash reads as `LOCAL_EDITED`, because hashes are computed after placeholder replacement. **A rerun of `setup-project` over an existing project used to replace a complete manifest with a near-empty one**, and under v3 that is data loss with teeth — a file absent from the manifest is project-owned by definition, so a rerun silently stopped syncing every template file it did not happen to rewrite. A rerun now updates the existing manifest instead of rebuilding it. Writing the parity rows also surfaced a pre-existing divergence between the two writers: bash's `$(…)` strips a trailing newline and PowerShell's `Get-Content -Raw` does not, so the two hashed different bytes for the same file.

**The reversal, stated plainly.** The plan and its spec had `CLAUDE.md` become fully template-owned, with project instructions moving to `.claude/rules/project.md`. **That is not what shipped, because it was measured unsafe.** A consumer session established, with the staleness confound controlled, that an unscoped rules file is delivered to nobody; that no rules content of any kind — scoped or not — is in a subagent's context at spawn; that a scoped file arrives only attached to a matching tool call; and that a missing `paths:` key means "matches nothing", not "matches everything". A `project.md` holding a consumer's instructions would therefore have reached no one, and the cutover would have deleted those instructions rather than relocating them. So **the PROJECT-CUSTOM region stays**, `CLAUDE.md` remains the only always-on channel, and `.claude/rules/project.md` is seeded as a `once` file whose own header states its limits: `paths:`-scoped conventions only, never safety rules, prohibitions, or which-tool-to-reach-for guidance. What the release still buys is real but narrower than advertised: every *other* template file gets a hash and a class, and is never three-way merged again.

### Deferred, named rather than silent

- **`CLAUDE.local.md` retirement → v3.2.** It is loaded at session start, which makes it a working always-on channel; replacing it with `paths:`-scoped files would remove one and substitute a mechanism measured not to deliver at spawn. Check 36's carve-out and the six-variant delete move together, since separating them turns the check red in between.
- **Check 37's bundle arm** asserts that a literal appears somewhere in a 436 KB file, where the literal occurs 19 times and at most 6 are the writer guard it names. It is live (stripping all 19 turns it red) but would not notice the guard changing shape.
- **Two sibling unquoted splits** in the lib remain unguarded. Inert today — both compare with `=`, and a value bearing glob metacharacters is not a legal refname — recorded so the glob fix is not read as closing the class.
- **`**Exclusive resource lock**:` and `**Worktree seed files**:`**, proposed by a consumer whose gate deploys into a live game install and corrupts running sessions when run at the wrong moment. A hook reading a declared lock path refuses mechanically where prose has to be remembered.

### Downstream migration

1. **`git add` and `git commit` must be separate tool calls.** The commit hook mints the gate artifact from the index as it stands when the call is submitted, so `git add newfile && git commit -m x` in one call mints a tree without the file and the merge gate then refuses. Staging before `run-gate.sh` is not required.
2. **Compound `checkout && merge` onto a gate-checked branch now refuses** and tells you to run the two as separate calls. This is the same rule as (1), for the same reason: these are PreToolUse hooks, so they see the state before your call runs.
3. **A glob in `**Gate-checked branches**` now matches branch names as written.** If you declared one and saw no gating, that was the bug; merges onto matching branches will start requiring a fresh artifact.
4. **SubagentStop eligibility moved into your agent files.** Add `pipeline: true` (echo + contract) or `pipeline: notify` (echo only) to the frontmatter of agents that should participate; an agent without Bash wants `notify`.
5. **`.claude/rules/*.md` needs a `paths:` key to be delivered at all**, and even then arrives only when a matching file is touched. Anything that must be true before work begins belongs in `CLAUDE.md`.
6. Re-running `setup-project` over an existing project is now safe for your manifest; if you ran it under v3.0.x and lost manifest rows, re-sync to restore them.
...
**A1 (yutraffic + open-brain).** `hooks/deny-secret-reads.sh`'s DENY text now names the matched token AND where it matched (`matched: "<token>" in argument <n>` for the Bash arm, `in the file path argument` for Read) instead of leaving a consumer to `grep -c 'Read(\.env' .claude/settings.json` for an explanation — two consumers were blocked within minutes of the hook shipping with no way to see why. A first draft pointed the false-positive remedy at "a script file, where the hook sees only `bash <path>`" and was rejected during review: that sentence is itself an evasion recipe against this exact guard (the hook scans the Bash COMMAND STRING; a script file's *contents* are not that string, so `bash audit.sh` containing `cat .env` would not be caught). The shipped remedy instead points at reading a config file with the Read tool, and a permanent suite row asserts the rejected phrasing never reappears. A SECOND draft's own added TEXT clause — "a literal secrets filename in any argument is enough, even when the command reads something else" — was caught at integration as FALSE against the hook's own source: the Bash arm is VERB-KEYED (`echo .env`, `ls -la .env` and `stat .env` are all 0, asserted), so any argument match is not enough. A deny message that misstates its own rule is worse than one that says nothing, because it is the artefact users reason from; the clause was removed. The replacement was then MEASURED rather than guessed: a consumer reproduced `echo 'Read(.env*) deny rules' && grep -c x settings.json` denying (2) while `echo .env` alone, `grep -c x settings.json` alone, `cat settings.json && echo hello`, and `ls -la && echo .env` all allow (0) — confirming both the reader-verb check and the secret-shape check scan the WHOLE tokenized command with no per-clause split, so a verb in one clause and a secrets-shaped token in another still fire together and a wrapper cannot hide the pairing across `&&`/`;`/`|`. Confirmed against the source before shipping, the deny text now states this shape explicitly (cross-clause matching is deliberate), and two permanent suite rows pin it: the cross-clause deny and a negative control where a reader clause carries no secret token.

**A2 (open-brain + panoscribe).** `hooks/run-gate.sh`'s `**Gate**:` extractor gets eight permanent marker-fixture rows (a real gate script that both writes a marker AND exits 1, so "ran and failed" is never confused with "never ran"). Five are CONTROLS that must never flip (an honest failing gate, a value with trailing `**Field**:` text, a bare-bash truncation shape, a double-star mid-value, and a `; true` tail — the last is deliberately an over-correction tripwire: it is the one row that goes RED if a future fix tightens the `**Gate**:` read into refusing any value containing `;`). Three rows exercise the actual regression: a trailing marker, with or without a `#`, in either field-name style (`**Gate**:` / `**Gate Command**:`), is part of the command once the read is anchored — restoring the pre-v3.0.3 greedy strip on a scratch copy flips exactly those three from (marker=1, artifact=0) to (marker=0, artifact=1: MINTED ON AN UNRUN SUITE) and leaves the five controls untouched.

**A3 (panoscribe).** `hooks/pre-commit-test.sh`'s `GATE_CMD_RAW` fallback extractor (the fifth site sharing the same anchored-grammar fix as v3.0.3's defect 3b) is now observable through its EFFECT rather than a new artifact field: a `**Gate**:` value that writes a marker file only if the WHOLE value runs, with the truncated tail (`true` alone) both exiting 0 and never writing it — the exact "looks fine, ran nothing" shape that hid this extractor's own defect. Zero hook change; the row does not depend on the thing it tests.

**A4 (panoscribe).** `hooks/run-gate.sh`'s header now documents that the TREE arm of `gate-before-merge`'s freshness check — the arm covering a rebase or amend that preserves content, which the SHA arm cannot — is defeated by any untracked file, because `tree` is computed via `git add -A` into a temporary index. Documented, not fixed: tracked-only hashing changes what the gate hashes and is a v3.1 design item.

**A5 (panoscribe).** All six `templates/*/PROJECT_CONTEXT.md` plus the root `PROJECT_CONTEXT.md` gain a one-line note next to the Gate field: join Gate command steps with `&&`, never `;` — `;` discards an earlier step's failure status, so `<real gate> ; <anything>` exits 0 and mints a pass artifact on a failing suite. No shipped default used `;` (verified before this change); this documents the trap rather than fixing a live default.

**A6 (open-brain).** `scripts/verify-template-consistency.sh` check 21 gains a reverse walk: every `hooks/**` file must have a same-named mirror in `user-level-reference/hooks/`, except an explicit `HOOKS_NO_MIRROR` allowlist (four entries: `enforce-delegation.sh`, `enforce-agent-contract.sh`, `agent-budget-warn.sh`, `require-skills-block.sh` — all project-team-model hooks with no meaning in a bare `~/.claude`). The new direction asserts NAME-PRESENCE only, not byte identity — this script is distributed to consumer repos via the sync-template skill, and a consumer's own project-owned "keep-mine" `run-gate.sh` may legitimately differ from the user-level copy on purpose; byte-comparing this direction would make every such consumer permanently red. A related finding: `retro-brief.sh` was previously unmirrored although it is machine-scoped exactly like the already-mirrored `retro-ledger.sh` (both key off cwd → project slug under the user's own `~/.claude/projects/`) — mirrored in this release rather than allowlisted, and a companion census (21c-2g) now asserts the two hooks' base-dir helper text stays byte-identical, since a divergence would make the brief silently read an empty ledger.

A verification note on how A2 itself was caught: a delete-the-guard verification grep with a backslash mismatch made both mutation checks silently SKIP while the suite reported all-passed; caught by reading the skip count on the first full run (945 with 2 skips), fixed and re-verified at 963/0/0 — the false-green shape these checks exist to prevent, found in the checks themselves. The durable rule: a suite's verdict is `passed == expected_total AND skipped == 0`, never `failed == 0`. A skip is a non-measurement wearing a green costume, exactly as an exit 127 is; asserting the EXPECTED TOTAL costs one integer and catches every silent-skip variant at once, including the ones not yet invented. `963/0/0` is a verdict; `all passed` is not, and neither is `0 failed`. The parser matrix already enforces this at release time (the node configuration asserts skip count 0 exact, the restricted configurations assert an in-band non-zero count), but Coder A's catch happened on a pre-matrix run — the rule applies to EVERY suite run, not only the matrix. No mechanical expected-total assertion is added to the gate in this release (scope freeze); it is a v3.1 item. Removing the offending string is a fix; excluding it is a standing exception that accumulates — the fix for a guard that fires is never to weaken the guard (panoscribe).

### Sync-template skill hardening — eight items from measured v3.0.3 consumer syncs

A new pre-step (§1b) refuses to sync from an untagged toolkit checkout, and when the drift is a docs-only descendant of a release tag with a byte-identical tracked template tree, records the TAG commit as `lastSynced` so the manifest label answers "which release am I on" rather than "which checkout sha did the server see" — the `describe` suffix is kept only when the template tree actually differs from the nearest tag (MM-Agent caught the sync about to record a docs-branch HEAD; penumbra's user rejected the resulting `v3.0.3-2-g5ce2d14` label on a sync that applied exactly v3.0.3). The step-2b planted-ignored-path control now appends with a leading newline and asserts the last pre-existing `.gitignore` rule is unchanged after restore — a `.gitignore` without a trailing newline made the old `>>` concatenate the control onto the last real rule, so the control reported "planted path not ignored" (the reassuring direction) while silently corrupting a rule for the check's duration (panoscribe). Step 4 defines merge correctness as placement AND presence: an added block must land under the same heading it sits under in the template — a `has_conflicts:false, dropped_lines:[]` merge placed the new Gate/Test comment 30 lines from the `**Gate**:` line it annotates, under `## Signal Plans` instead of `## Commands`, and every existing check passed; heading anchoring is used because the field line itself differs between template (`{{GATE_COMMAND}}`) and project, so an exact-match anchor finds nothing (yutraffic; the real three-way fixture ships under `scripts/fixtures/b3-placement/`, red on the misplaced merge, green on the correct splice). Step 6b compares tracked-ness against the manifest PLUS this sync's applied set, because a file applied in the same sync reads PRESENT-BUT-UNTRACKED until finalize writes the manifest (yutraffic). The `/tmp`-is-invisible-to-Windows-`python3` warning is generalised from step 2b to every bash→python handoff (panoscribe). Step 7b's version-label stamp is confirmed LOAD-BEARING against the server source — `template_finalize_sync` mutates only `lastSynced`, `version` and `files` and never writes `lastSyncedVersion`; a manifest that already shows the fields was stamped by a previous 7b (open-brain, panoscribe). A migration note states that verifying the retirement of the `Read(.env*)` deny rules trips the hook that replaced them — `deny-secret-reads.sh` judges the command TEXT, so `grep -c 'Read(\.env' settings.json` is blocked — and that the remedy for inspecting a non-secret config file is the Read tool, NOT a script file, which would be the exact bypass the hook exists to prevent (open-brain, yutraffic). And hook-file writes during `/sync-template` are documented as unrunnable while a live process polls Bash/PowerShell: the broad-matcher `gate-before-merge.sh` stays lock-held and the atomic rename fails `WinError 5` (MM-Agent, three times running) — the discriminating fact is that only the hook wired to the broad `Bash|PowerShell` matcher is lock-held near-continuously, so a consumer seeing four hook writes succeed and one fail should not read the failure as transient (MM-Agent). A fixture pushed without `-u` has no upstream set, so a bare `pull --ff-only` refuses for a reason unrelated to the rule under test and reads as a false regression — push `-u` on a fixture's first push, now the fourth named trap in the presync verification section (penumbra).
...
**Two root causes, told apart by their symptom.** (a) The six deny rules above — the prompt says *"a `Read()` deny rule is configured; only you can approve running it anyway"* and offers no "switch to auto" option. Consumers whose `settings.json` is PROJECT-CUSTOM must delete the six lines by hand; the sync skill will not do it for them. (b) `autoMode.environment` is **user scope** and applies to every project, so a repo-specific "Trusted repo" line there makes the classifier deny ordinary commands in every OTHER repo — symptom: a classifier denial in the transcript, and the pause after 3 consecutive / 20 total. `setup-project.{sh,ps1}` now print the generic snippet and `verify-user-level-drift.sh` compares `autoMode.environment` against the reference.

### 8b. The four verbs that read `.env` from inside the hook's own scope

A consumer measured `rev .env`, `dd if=.env`, `cp .env /dev/stdout` and `cat .e*v` all returning 0 against the new hook. The proposed remedy — deny any argv token naming a secrets file, regardless of verb — is the rule the hook's own header had already considered and **rejected**, because it denies `cp .env.example .env`, `git add .env` and `rm .env`, none of which are reads, and *a missed read is a gap; a denied write is a guard people switch off*. It stays rejected. The holes are closed inside the verb model: `rev` added to the reader list; operands read out of INPUT-shaped `key=value` keys (`if`, `*file`, `*input`, `*in`) so `dd if=.env` is seen while `grep --exclude=.env` stays allowed; a copy class (`cp`, `install`, `rsync`) denied **only** into `/dev/stdout`, `/dev/fd/1` or `/dev/stderr`; a transmit class (`curl -T`/`--data-binary @`, `scp`, `wget --post-file=`) because sending the contents off-machine is a read by another name; a case-insensitive match; and a stated **heuristic** for globs — under a listed verb, a `.e`-prefixed token containing `*`, `?` or `[`. Writes and exclusions have want-0 fixture rows so the rejected rule is asserted, not just argued.

### 8c. `git -C` resolution: a global option before `-C` made the repo invisible

**Since v3.0.1** (`hooks/lib/git-cmd.sh` is byte-identical across v3.0.1 and v3.0.2). A global option before `-C` — `git -c a=b -C <repo> merge`, `git --no-pager -C <repo> push` — made the shared lib return no path, so `gate-before-merge`, `no-push-main` and `pre-commit-test` judged the **payload cwd** instead of the named repo: exit 0 from an unprotected cwd, and 2 from a protected one by luck rather than by judgement. This is **separate from finding 62** above and has the same cause shape: that defect stopped the clause MATCHING as git; this one stopped the repo RESOLVING.

`hooks/lib/git-cmd.sh` now consumes every leading global until the first non-option token and folds every `-C` in argv order — absolute overrides, relative composes, measured against git 2.55.0 — and stops at the subcommand so `git commit -C <commit>` is never read as a directory change. The local `a6_repo_for` fold in `gate-before-merge.sh` is **deleted**: it fixed one of three callers and could not fix the order rule at all, which lived in the lib. The rule in precedence order: when the final folded path resolves it is judged **as git would resolve it** (relative composes, absolute overrides) — so `git -C <protected> -C <unprotected>` is allowed, because that is where git lands; the **strictest resolved candidate** is judged ONLY when the final path does not resolve; refusal only when **no** candidate resolves, naming the operand (`hook could not resolve -C <operand>`). A single `-C` into a missing directory keeps its documented exit 0. `--attr-source` joins the explicitly refused globals — it changes which tree gitattributes resolve from — instead of being caught by the unknown-global default.

**And a path containing `-C` inside a word made the local fold return empty** — the third defect in the same function, found only because one consumer's fixture reproduced while two others' did not, on the same machine. The deleted `a6_repo_for` extracted its operands with `sed 's/.*-C[[:space:]]*//'`, whose leading `.*` is greedy, so it cut at the LAST `-C` anywhere in the string. A scratchpad under `…/G--git-Yutraffic-Challenge/…` gave `hallenge/…/w` — a relative path that does not exist — `gc_resolve` fell back to the base, and the hook judged the payload cwd on **every** cell. The token walk that replaces it is insensitive to path content, and there are fixture rows on all three hooks using operands that carry `-C` inside a word (`Sub-Challenge`), `git` as a path component and inside a hyphenated name, and a directory named `merge`.

`scripts/test-hooks.sh` carries a labelled REGRESSION block for the multi-`-C` shapes that already passed via the local fold, order rows from **both** cwds on all three hooks, a two-globals row, the `git commit -C HEAD` control, and the strictest-wins / refuse pair. Reported by yutraffic, panoscribe and open-brain against 0d7806e.

`hooks/lib/` changed, so `scripts/test-hooks-parser-matrix.sh` is mandatory before the tag.
...
A `-C` operand whose path contains `[`/`]` was reported to bypass all three git hooks. Re-measured with a valid fixture: the earlier repo was built via `git clone` INTO a bracket-named destination, which git-for-windows creates under an MSYS-converted name the shell's own literal string cannot address, so the fixture repo was never reachable by the path the test asserted against — the same "repo without commits (reachable at that path)" trap as other findings this release. Built instead with `mkdir` + `cd` into the directory + `git init` with no path operand, `w[1]`-style paths are gated identically to a non-bracket control on all three hooks, single and double `-C`, both fold orders, with the unprotected-target mirror at 0 — rows added to `scripts/test-hooks.sh` under `(A6.bracket)`. `hooks/lib/git-cmd.sh` is unchanged.

### 12. FINDING 63 (round 2) — `gc_push_args`'s greedy fallback discarded the refspec it was reading

The primary `sed` in `gc_push_args` permitted at most one `-C`; a double `-C` failed it, and the fallback (`sed -n 's/.*\bpush\b//p'`) stripped GREEDILY through the LAST word-bounded `push` in the segment — which any trailing token merely CONTAINING "push" as a whole word carries (`--push-option=ci-skip`, `--receive-pack=/x/push`, `-o push-me`, or a refspec branch literally named `feature/push-fix:main`). Measured: from an unprotected cwd, with the target repo on a FEATURE branch (main protected), `git -C /tmp/other -C <P> push origin other:main --push-option=ci-skip` returned rc=0 through `no-push-main.sh` — a real push landing on protected `main`. With the target checked out to `main` the row already returned 2 (the current-branch fallback caught it by luck, not by reading the refspec), which is why the fixture needs the target OFF `main` to fail at all — proved by re-running the same row with the target on `main`: still 2, both before and after this fix. `gc_push_args` is rewritten to walk the token stream POSITIONALLY (the same global shapes `gc_dash_c_list`/`gc_global_options` recognise), returning everything after the FIRST token that IS EXACTLY `push`; the greedy fallback is deleted, not repaired. An unrecognised bare word is SKIPPED rather than aborting the scan — this keeps covering the quoted-path-with-space shape (`git -C "C:/a b" push …` word-splits into `-C`, `C:/a`, `b`, `push`, …) the deleted fallback used to catch, now proven by a regression row rather than by construction alone.

### 13. FINDING 64 — the `-C` resolver was a strict SUPERSET of git's unresolvable set: a silent bypass pre-existing since v3.0.0, and a false refusal new in this release

The hook receives the command PRE shell-expansion. `gc_resolve`'s absolute arm (`/*|[A-Za-z]:[/\\]*`) never matched `~/…` or `$VAR`, so those took the relative arm, `[ -d "$base/~/repo" ]` was false, and the operand fell back to `<base>` — while the REAL shell expands `~`/`$VAR` before git ever sees them and git resolves the true target. **This half is PRE-EXISTING, not new to this release: `gc_resolve` is byte-identical back through v3.0.0.** `git -C ~/protected push`, judged against the wrong (payload-cwd) repo, has landed silently in the real protected repo since v3.0.0, with no message at all — the single-`-C` case was never even reached by the multi-`-C` refusal check (`gc_dash_c_unresolved`'s old `>= 2` guard), so nothing in that machinery could have caught it either.

The **other** half is new to THIS release: v3.0.2 had no `gc_dash_c_list`/`gc_dash_c_unresolved` at all — that machinery was added earlier in v3.0.3 (item 8c/finding 63) to fold repeated `-C`. Once it existed, a double-`-C` `~`/`$HOME` command that git executes fine (`git -C ~/a -C ~/b commit`) was refused outright, because neither operand matched the old absolute-path test — a FALSE refusal this release itself introduced.

**The fix is a three-class operand classifier** (`gc_classify_c`, applied to BOTH `gc_resolve` and `gc_dash_c_unresolved`, single- and multi-`-C` alike — the old `>= 2` guard on the refusal check is gone, replaced with "no `-C` at all → no refusal", the correctness condition its removal now depends on):
...
Class 1 (literal path, existent or not) keeps the documented `[ -d ]` test and the pre-existing exit-0 fallback on a single `-C`. Class 2 is resolvable by STRING SUBSTITUTION ALONE — no eval, no unquoted `~`, no `$(...)`, no `sh -c`: `~` / `~/…` substitutes this hook process's own `$HOME`; `$HOME`/`${HOME}`/`$USERPROFILE`/`${USERPROFILE}` substitute the matching value from the hook's own environment (a user property, identical across the hook and the payload's shell); `$PWD`/`${PWD}` substitute the PAYLOAD's cwd (`<base>`, already passed to every caller) and are NEVER read from the hook's own `$PWD` — the hook runs where the harness started it, not where the command runs, and reading the hook's own `$PWD` would reintroduce the exact bypass this fix closes while looking like a resolved answer instead of a visible fallback. Class 3 (cannot-determine) is anything else carrying `$`, `` ` ``, `$(`, `<(` or `>(`, PLUS `~user/…` — and **keys on the PRESENCE of the metacharacter, not on whether it would actually expand**: the hook payload does not carry which shell ran the command, so "would this resolve" is unanswerable from here, but "does this look like something only a shell could resolve" is. Under bash `~` expands (the bypass this closes) and refusing an unresolvable one is correct; under PowerShell `~` is literal and git fails on its own, so refusing it there is harmless — refusing something that would have failed anyway. `%VAR%` (cmd.exe style) is deliberately left OUT of class 3: neither bash nor PowerShell expands it, git fails to chdir into the literal string, and that is the documented class-1/single-`-C` fallback, not a new case.
...
**Scope is `docs/**` AND `user-level-reference/**` (26 docs), because that is where the motivating case lived.** The reference that proved the gap existed was step 9 of `skills/sync-template/SKILL.md`, and a first cut scoped to `docs/` alone **could not see it** — a check blind to its own motivating case is the adjacent-question defect one more time. Broadening required excluding **fenced code blocks**, which is correct on its own merits and not a workaround: a line inside ``` or `~~~` is a sample, a transcript or a **measurement** — never live routing prose. This skill's own noise census (`doc-generator 10 | test-writer 4`) is a recorded measurement; flagging it would demand falsifying a number that was true when measured. Prose one line outside the fence is still flagged. Dated historical records (`docs/plans/**` and `docs/<YYYY-MM-DD>-*.md`) are excluded **by shape**, not by a list that rots.

Six arms, because fence-tracking can fail *silently*. **B** asserts a synthetic *unmarked* line IS flagged; **C** asserts the same name *with* a marker is NOT — positives alone cannot tell a working guard from one that fires on everything. **D** asserts neither input is empty (and it earned its place immediately: it caught a broken pathspec that had silently cut the scan from 26 docs to 2). **E** is the two-sided control on the fence extractor itself — the fenced hit must be invisible *and* the prose hit one line later must not be, because **extracting nothing at all is the clean-looking failure**. **F** asserts fence delimiters are balanced in every scanned file: an odd count leaves the tail of a file permanently "inside a fence", so live prose after it is never scanned and arm A returns clean. That is the one way the exclusion turns into a false negative, and it is now loud.

`user-level-reference/README.md` also named `requirements-engineer` and `test-writer` as a live agent roster; it now names the survivors, with the original roster preserved as an annotation.
...
5. **Check 32 may be red on YOUR docs, and that is the check working.** If your own `docs/` or `user-level-reference/` name `test-writer`, `requirements-engineer` or `doc-generator` outside a code fence and without a retirement marker, it will say so with `path:line`. Fix by naming the survivor (`tester`, `architect`, `coder`), or — if the line is a record of what was true then — **annotate it rather than rewriting it**. Do not loosen the matcher. Arm F may also go red on an **unbalanced code fence** in one of your docs: that is a real markdown defect, and everything after the unmatched delimiter was invisible to the scan.
...
"Defined and reported but not written" needs one clarification, because on its face it contradicts "the client cannot observe the server's version": **"reported" means the summary prints a labelled slot with an explicitly unknown value** (`Sync server: unknown`), not an observed one. The slot is the deliverable; the value is honestly absent.

**`lastSyncedVersion` resolved to a bare `""` on exactly the hosts that most need it — item 1 reproducing its own problem one level down.** Client-side resolution needs the toolkit checkout present **with its tags fetched**, which is the very condition this item opens by saying cannot be assumed, and `""` is indistinguishable from "resolved, to nothing". Four distinct outcomes now get four distinct values: an exact tag; a suffixed `describe` form for an untagged commit (a real, precise answer, not a failure); `unknown:no-checkout`; `unknown:no-tags`. The key stays **absent** where this skill never ran. Bounding datum: on a host with the checkout and its 18 tags fetched, `--exact-match` resolves correctly — the failure is bounded to no checkout, a shallow clone, or a fetch without tags.

**A hazard that does NOT fire, pinned so it cannot start.** `git describe --tags --abbrev=0 <sha>` on an untagged intermediate returns the nearest **ancestor** tag — a bare `v2.2.4` for a commit strictly *newer* than v2.2.4 — so the manifest would record a version **older than the one applied**, with no error and no empty string, on exactly the consumers who sync off-tag to pick up a fix early. `""` is honest; a nearest-ancestor tag is confidently wrong. What ships is `describe --tags` **without** `--abbrev=0`, which yields `v2.2.4-3-gabc1234` and is self-evidently intermediate — but it is one "tidy-up" away, since `--abbrev=0` is what someone reaches for when the suffix looks like noise. A comment at the call site now says the omission is deliberate and why, with a two-arm control: a tagged HEAD resolves to the exact tag, and an untagged HEAD produces a value **matching `-g[0-9a-f]{7,}`**. That second arm is asserted **positively**, not as "is not an ancestor tag" — the negative phrasing passes vacuously on an empty string, a malformed value or a swallowed exception, giving a fixture the *shape* of a failing-arm test whose failing arm can go green for unrelated reasons. Every other pair in this release was re-checked against that test.
...
**The hoisted invariant is NOT what fixes this, and the evidence is the reporter.** An invariant of exactly this shape *already existed* — step 6b rule 4 states the prohibition for this class in near-identical terms, they had read it, and they still called `source="template"` in step 5. The invariant is an abstract prohibition at the top; the action is a concrete imperative naming a tool call, minutes of reading later, and **when executing literally the imperative at the point of action beats the rule stated elsewhere**. Adding a second invariant would repeat the mechanism that just failed. So step 5 is **restructured to make the destructive call unreachable**: it branches on disk presence first, and `source="template"` appears only inside the "does not exist" arm. A reader following it literally *cannot reach* the wrong call rather than being expected to remember not to. **I1** — a file that exists in the project is never written from the template without an explicit conflict resolution — remains, as a backstop, alongside **I2** (see item 8). Scope is confirmed complete: every other `source="template"` site in the skill was audited; step 3 is manifest-tracked by definition and 6b step 3 is explicitly guarded by MISSING. Step 5 was the only hole.

**And a present-on-disk path is now surfaced as a CONFLICT, not a silent skip.** Skipping is safe but it *hides* that the template ships a different version of a file the project already has — semantically both sides have content, which is the definition of a conflict. Under a silent skip the consumer who lost the file would have been told nothing and would never have learned the template's `run-gate.sh` existed and differed — **which is the comparison that produced item 7 of this release.** So the normal three options are offered, **defaulting to keep-mine**, with the diff shown: safe by default, and the user learns the template has a version. Every such path is listed individually under `Skipped:`/`Conflicts:` — never as a count, which is how the one file someone cared about goes unnoticed. `source="skip"` remains the mechanism for the keep-mine outcome: it registers a manifest entry from the project's own content *without writing*, so future template moves arrive as ordinary conflicts with real diffs.

**Bootstrap is the worst case and the original writeup did not cover it.** A fresh bootstrap has an empty manifest, so **every hand-written file in the tree is simultaneously present-on-disk and absent-from-manifest** — the entire destructive intersection at once, on the one path where step 2b's manifest-scoped backup protects nothing at all. Now stated explicitly, and covered: **step 2b's backup set is extended to `new_template_files` ∩ exists-on-disk**, which is the only mechanism on the table that reaches the gitignored member of that class — the file with no history, no diff and no undo. 2b is *not* reordered to make that work: measured against the shipped skill, 2b already sits after Compute Status, so `new_template_files` is in hand. (The confusion came from its *label*: "2b" reads as belonging to step 2's inputs when it is sequenced after step 2's output. Moving a step whose whole contract is *before any write* is exactly how the next data-loss bug gets introduced.)

**A post-apply guard, and two ways it could have shipped broken.** After step 5's last write, every keep-mine path is byte-compared against its 2b backup.

- A `git status --porcelain` invariant was proposed first and **rejected**: porcelain does not list ignored files without `--ignored`, so it is blind to precisely the severe case — present on disk, manifest-untracked **and gitignored**. It would have had overlapping coverage of the recoverable case and zero coverage of the unrecoverable one. The byte comparison does not depend on git's visibility rules at all.
- Keyed on the **path list**, the guard reproduces the very bug it closes: a user who deliberately chooses accept-template *should* see the bytes change, and the guard would fire on a correct, user-approved outcome — after which someone weakens it and the hole reopens. **That is how item 4 happened in the first place**: two rules, each right in isolation, contradicting for one class. So the guard is scoped by the **recorded resolution** — the manifest already carries `"resolution": "keep-mine"` where `source="skip"` set it, and omits the key elsewhere (verified populated on a live consumer manifest). Step-4 keep-mine files are explicitly out of scope: they are ordinary tracked paths that 2b does not back up, and pulling them in would hard-error a correct sync.

> **The durable rule from this round, stated in the skill next to both guards: a guard must be keyed on the DECISION that was made, not on the list the file arrived in.** A list describes provenance; a guard enforces intent. It outlives every specific fix here — item 7's clamp is the same rule one layer down.

**The guard itself could fail by reporting success**, which is the signature of the bug it guards: a byte comparison against a backup that does not contain the path finds nothing to compare and passes. So it asserts the backup **covers** each path first, and a missing backup entry is a **hard error, not a clean result** — with the reason in the step text, or the next reader deletes it as a redundant existence check.
...
**Keeping stderr visible is necessary but not sufficient — the EXIT CODE hides too.** A malformed `grep` invocation exits **2**, but arm 2 is a pipeline, so a bare `$?` is the *last* grep's status and the 2 never surfaces: anyone wrapping the sweep in a script that tests its result gets a clean-looking answer even with stderr unsuppressed. `set -o pipefail` (or a `${PIPESTATUS[0]}` check) now ships with the sweep. Stderr caught this for a human; the exit code is what catches it for a script.

**A whole class both arms are blind to, decided deliberately rather than by omission: placeholders inside FENCED CODE BLOCKS are OUT OF SCOPE.** A reviewer scanning unfiltered found a consumer's `CLAUDE.md` carrying `{{BUILD_COMMAND}}`, `{{TEST_COMMAND}}` and `{{FORMAT_COMMAND}}` unfilled inside its Quick Start block — arm 1 misses them (not a `- **Key**:` line), arm 2 misses them (not `.sh`), and fixing `--include` does not touch it. **The decision is to leave them out, and the skill now says so with the reasoning.** First, it is the same constraint that narrowed arm 1 to begin with: a pattern broad enough to reach fenced blocks flags the toolkit's own *documentation* of placeholder handling — including the sweep's own source and every template's Quick Start — and the better the handling is documented, the noisier its own detector becomes. Second, the failure this sweep exists for is a config value read as data by a **tool**: v2.2.0's `- **Protected branches**: {{DEFAULT_BRANCH}}` silently unprotected trunk. A fenced snippet telling a human what to type is read by a human, who has the surrounding sentence — in the reported case the file's own "Replace placeholders above with your project's actual commands." The residual gap is **named** in the skill rather than left implicit, and the fact that would reverse the decision is stated: a fenced-block placeholder that some tool parses. There is not one today.
...
All the bare-name rows are identical, so the check could not distinguish anything — a consumer following step 4 would go hunting a syntax error in a file that is fine. Same class as the `command -v python3` App-Installer stub: the name resolves, the program is not the one you meant.

**Two things this entry originally said were wrong, and both are corrected here rather than smoothed over.**

**(a) The prescribed fix hardcoded `C:\Program Files\Git\bin\bash.exe` and warned against the `usr\bin` path `shutil.which` reports. Both halves are withdrawn.** The last two rows above are the comparison that warning was written without ever running: `bash -n` parses without executing, needs no path translation, and the two binaries are indistinguishable. And the hardcoded path **does not exist on Linux, WSL or macOS**, so shipping it makes the check unrunnable everywhere except this one host — while any fallback to the bare name restores the bug it was meant to fix. An unmeasured warning is worse than no warning: it teaches the next reader to distrust the correct binary. The root cause is narrower than the fix assumed — **`shutil.which(name)` and `subprocess.run([name])` resolve the same BARE NAME by different rules; they cannot disagree about an absolute path** — so the shipped fix is one host-agnostic line with no platform knowledge in it: `exe = shutil.which("bash")`, then only ever exec `exe`.

**(b) "Treat `rc=127` as the check did not run" was a FAIL-OPEN, and would have been the eleventh in this series.** `rc=127` is bash's own verdict *about the file it was handed* — "No such file or directory". It is not "the interpreter failed to launch". As written, the rule waves through `bash -n hooks/deleted-thing.sh` on a manifest still referencing a script that is gone — **a missing hook script is exactly the condition this toolkit's 127-wrapper exists to catch**, the one mechanism the v2.2.1 entry calls "the only thing that turns a fail-open into a fail-close". Shipping it would have reinstalled that fail-open one layer up, inside the checker, in the release that adds a reserved code for making failure modes distinguishable.

The distinction actually wanted is **structural and needs no exit code**: "the interpreter did not launch" is an *exception*, not a return code — Python raises `FileNotFoundError` before any rc exists (verified). The skill now ships this table, and `hooks/lib/git-cmd.sh` states next to its own 127 line that the hook meaning does not carry across to a checker you invoke yourself:

```
FileNotFoundError  -> could not check; name the interpreter. THIS is "did not run"
rc 0               -> pass
rc 2               -> syntax error — the ONLY fail verdict
rc 126             -> could not READ it (permissions, or a DIRECTORY passed where a
                      script was expected). ERROR; print the path
rc 127             -> bash could not find THE FILE. ERROR; print the path
anything else      -> unknown. ERROR
```

`rc 126` was unhandled everywhere before this round — a directory passed where a script was expected lands there rather than at 0, so it is now an explicit arm.

**Process note, because this is the second time the same move produced a wrong instruction.** Round 2 measured the datum that contradicts (a) — it recorded that `usr\bin\bash.exe` by explicit path also returns 2, i.e. the breakage is the PATH lookup and not that binary — and then shipped the unchanged fix anyway. The datum was one step short of its own conclusion. **A brief that describes code is a secondary source**; the cheap discriminator is *did this come from the thing, or from a description of the thing?* Every wrong instruction in this release shares that tell: an unrun comparison, an unopened file, a label inference, a probe's own flag read as the implementation.
...
A hard-blocked repo and circular advice. **The defect is not the wording** — the outer handler had no way to tell a *terminal* failure from a *retryable* one, so rewording this guard would leave every future guard to hit the same wall and every outer handler to remember a special case.

**THE BRANCH IS THE FIX; THE NUMBER IS ONLY A NAME — and the brief had this backwards.** `run-gate.sh`'s self-reference guard has exited a distinct, non-generic code (`2`) since **v2.1.3**, and no outer handler ever read it: before this release **no script in `hooks/` captured `$?` into a variable or compared an exit code against anything at all**. The advice was circular not because the code was indistinguishable — it already was — but because **nothing looked at it**. So the load-bearing half of this item is `pre-commit-test.sh` learning to *capture and branch*; renaming 2 to 78 while the callers still tested truthiness would have changed nothing observable and shipped the same circular message with a fresh number. The rename is still right, for a different reason than the one first given: `2` is already the hooks' own **block** code, so the moment anything does branch, a terminal *gate* condition and an ordinary *hook* block would be indistinguishable.

> **`GC_TERMINAL_RC = 78` means TERMINAL. The `sysexits.h` name is a coincidence, not a rationale.** The original justification — 78 is `EX_CONFIG`, "configuration error" — is withdrawn twice over. First, the class is wider than configuration: the self-reference guard *is* a config error, but "not inside a git repository" and a consumer's missing-dependency preflight are **environment** errors; all three are terminal and only one is a config error, so anyone checking the number against `sysexits.h` would conclude two codes are needed and re-fragment the class this exists to unify. Second, a *documented* code is **more** likely to collide, not less, because other sysexits-aware programs emit 78 for its documented meaning. All meanings live together in `hooks/lib/git-cmd.sh` — including the note that `0` is overloaded at the success end (passed **or** did not run), which v2.2.4's BOM bug is exactly the mechanism for making live. Not fixed here; documented so the next person adding a code sees the whole shape.

**78 is an INTERNAL signal and never a hook's own exit status.** The harness treats every non-zero, non-`2` PreToolUse exit as a *non-blocking* error and lets the tool call proceed — so a hook that *exits* 78 warns and **allows**. `run-gate.sh` may return 78 because it is a runner, not a hook; `pre-commit-test.sh` still exits **2** when it blocks and uses the 78 only to pick which remediation text to print. Both arms are asserted.

**The clamp — and it is what makes 78 safe, not the number.** Until this release every nonzero from the gate command collapsed to a hardcoded `exit 1`, because `$?` was never captured. *That was already a clamp, and it existed by accident.* Giving the guard a distinguishable code is precisely the change that leads someone to refactor that line into `rc=$?; exit $rc` — at which point a consumer gate command exiting 78 for its own reason inherits the terminal remedy *"edit your `**Gate**:` value"*, printed over a plain test failure. That is **inverted** advice, strictly worse than the generic retry line it replaced, and quieter. Real: a peer's `scripts/gate.mjs:43` is `process.exit(res.status ?? 1)`, the child's code verbatim, and `uv run` propagates a child's code exactly. So the clamp is now **explicit and controlled**: a gate command's 78 is mapped to 1 before it is read.

**Which creates a contradiction the brief did not see, and it is resolved with provenance rather than with the number.** In the motivating case the gate command **is** `run-gate.sh`, so the terminal 78 arrives *through* the clamp — clamping it unconditionally would swallow the one signal this item exists to deliver, and turn the existing two-arm fixture red. The exit code carries a **value**; what the outer layer needs is **provenance**. So the recursion guard touches a marker file inside the outer run's temp dir (`RUN_GATE_TERMINAL`, exported before the gate command runs and inherited at any nesting depth, including through a wrapper), and the clamp fires only when the marker is *absent*. Keyed on **who decided**, not on the number — the same rule item 4 below states for the sync skill. Zero false positives in either direction, and no substring guess about what the gate command "looks like".

**There was a SECOND terminal guard already in the file, and shipping without it would have fixed one instance of a class this release names.** `run-gate.sh`'s *"not inside a git repository"* exited **1**, so a consumer running the gate from outside a checkout got `GATE ERROR: not inside a git repository` followed by `re-run it and fix the failures` — the identical circular advice, in a guard that already existed rather than a hypothetical future one. Re-running from the same cwd cannot make that directory a repository: it is terminal, and it moves to 78 in the same commit, with a remedy line naming the fix. Its sibling `cd "$REPO_TOP" || exit 1` **deliberately stays 1**, and the file says why: a `cd` failing on a path git *just* resolved is a transient environment fault — a race, a permissions change, an unmounted share — where retrying can legitimately succeed. That sentence is what stops someone "consistency-fixing" it later. A third member of the class exists in the field (a consumer's `pytest-cov` preflight, remedied by `uv sync`, exiting 1 today); they will move it to 78 *after* the outer handlers ship the branch, not before, which is the correct sequencing.
...
The census asserts **name and value**, not the name alone — `grep -qF 'GC_TERMINAL_RC=78'` in both files — because two copies that are *different numbers* pass a name-only census as readily as one. (The `GC_KEY_PRE` census it is modelled on was already value-checking; the pair of name-only checks next to it are *bypass* censuses over call sites, where the name is the right thing to look for.)

Fixtures assert **both** arms of every guard, because a one-armed fixture cannot catch a suppression that fires on everything: terminal (78, no generic advice, remedy last, `pre-commit-test.sh` blocks 2 without retry advice) and ordinary red (`run-gate.sh` still exits 1, both layers still print the retry advice); a gate command exiting 78 for an **unrelated** reason clamped to 1 with the retry advice restored, against the nested case where the 78 must survive; and the not-a-git-repository guard returning 78 against the ordinary red gate returning 1. Each was verified **by deleting its guard** and confirming the control goes red — a control that still passes with its guard removed is decorative and tests the surrounding machinery instead.
...
- **New files arrive after `settings.json` in the numbering, and 9 of 11 of them were hooks it wires.** Following step 3's "settings.json last of the enforcement wiring" literally, then adopting new files in step 5, installs a `settings.json` naming nine scripts that do not yet exist — every matching tool call exits 127 and fails closed. The reporter ignored the numbering and applied libs → hooks → agents → settings.json → docs, which is right. Now stated as invariant **I2** — and as a **PRECONDITION checked at the point of the write**, not as an ordering rule: before writing `settings.json`, verify every `hooks/` path it references exists and is non-empty, and refuse otherwise. An ordering rule silently degrades the next time a step is inserted or renumbered, which is exactly how this bug arose; a precondition does not. (Untested from the field: none of the reporting consumer's four syncs brought new hook or lib files, so nobody has confirmed or refuted the ordering break in practice.)
- **Step 9's own commit is blocked by the gate it just installed** — the sixth instance of this trap this week. `gate-before-merge.sh` scans the whole command string, and a template-sync commit message *naturally describes merge-gating changes*. Step 9 now prescribes writing the message to a file with the Write tool and using `git commit -F <path>`; the short `-m` it used to carry dodged the gate by luck, not design. **The same trap sits one command later in `gh pr create --body`**, and a sync PR body describes gate changes by its nature — `--body-file` or `--fill` instead. One reviewer dodged it only by using the GitHub MCP tool rather than `gh`, which is luck again, so the skill now states the general rule: **any text describing gate changes goes in a FILE, never in a command string.** Field evidence in favour: one consumer already writes the sync commit message to a file and uses `git commit -F` on every sync — four running — precisely for this reason. The prescription documents existing practice.
- **Step 2b's assertions are now labelled by what each one can tell you, and the non-empty one gets a control.** The `\r` defect that v2.2.4 fixed was caught by the three-method **cross-check**, not by the non-empty assertion: one method disagreed with two others, which *localised* the fault to path hygiene. The non-empty assertion would have fired on the same defect and reported only "empty", sending the consumer hunting a manifest bug that did not exist — a path-hygiene artifact wearing the costume of a real finding. Both stay, with that difference stated: **it tells you something is wrong, not what.** It has never fired in four syncs, which is what an invariant is supposed to do — and it is the only guard that survives both cross-check arms being wrong *together*, since both call `git check-ignore`. It stops being untested cheaply: plant a known-gitignored temp path and a known-not-ignored one, assert the first comes back and the second does not. **Both arms**, because a positive-only control passes just as happily against a derivation that returns every path — and this step explicitly permits a superset fallback, so that is a live failure mode, not a hypothetical one.
- **The item this round expected to find here does not exist, and saying so is the point.** A defect was reported in step 2b's shipped example code — a Python one-liner printing paths in text mode, emitting `\r\n`, which `mapfile -t` strips only `\n` from. **Step 2b ships no such snippet.** Since v2.2.4 it prescribes `sys.stdout.buffer`, carries the CR guard verbatim, and names Method 2 as the more exposed of the two precisely because its path source is left to the implementer. Reported as a contradiction rather than "fixed" by inventing the defect; what survives of the item is the two lessons above.
...
### 9. Round 4 — the boundary that was not clamped, two warn-and-allow exits, and a label that goes stale after being right

**`pre-commit-test.sh` clamped one path and not its parallel.** The run-gate path is safe because `run-gate.sh` clamps its own gate command's 78 internally, keyed on the provenance marker it created — so a 78 emerging from it has already been *decided* to be a terminal guard talking. The `eval "$TEST_CMD"` boundary has nothing behind it: `$TEST_CMD` is a consumer's `**Test**` value, or the raw `**Gate**` value when `run-gate.sh` is absent beside the hook, and 78 is `EX_CONFIG`, which real programs emit. **So the identical gate command got opposite remediation depending only on whether `run-gate.sh` sat beside the hook** — contradicting the conventions block shipped in the same commit (*"a hook NEVER forwards a child's exit code"*), and the same *fixes-one-instance-of-a-class-it-just-defined* shape that forced the second terminal guard into round 3. The boundary is now clamped, the terminal arm it made unreachable is deleted rather than left as a branch nothing can observe, and **the asymmetry between the two boundaries is stated in the file** because it is real. One residual edge is recorded rather than engineered around: `**Test**: bash hooks/run-gate.sh` over a self-referencing `**Gate**` loses the terminal *framing* (never the guard's own remedy, which still prints) — two pathologies at once, and attributing across a boundary this hook did not create would mean rebuilding a causal chain out of a file.

**`exit 1` from a PreToolUse hook is warn-and-ALLOW — the twelfth instance of this release's defining shape.** The harness treats every non-zero, non-2 PreToolUse exit as a **non-blocking** error and lets the tool call proceed. Both `cd "$REPO_PATH" || exit 1` sites in `pre-commit-test.sh` therefore let a commit through **ungated** when the resolved repo path was unreachable: cannot-determine permits. Pre-existing, and in `main` too — but only *knowable* since this release documented the harness contract. Both are now `exit 2`, naming the path. **Deliberately 2 and not 78**, on the axis round 3 settled: a `cd` failing on a path git just resolved is a transient **fault**, not a settled condition, so "re-run it" is honest advice — which is also why `run-gate.sh`'s sibling `cd "$REPO_TOP" || exit 1` correctly stays 1 (it is a runner, not a hook). Reaching either site needs the directory to vanish between the `PROJECT_CONTEXT.md` grep and the `cd`, a race no fixture can honestly drive, so the property is asserted **structurally**: no `exit 1` may appear in a registered git gate.

**A stale version label is worse than an absent one, and step 7b's never-clobber rule guaranteed staleness.** Rule 2 (*"only if the key is absent or empty — the client never clobbers a value it did not write"*) applied to **both** version fields. Correct for `templateSyncToolsVersion`, which is server-owned; **wrong for `lastSyncedVersion`**, a pure function of `templateRepo` + `lastSynced`, and `lastSynced` changes on every sync. Harmful or vacuous with no third case: if `template_finalize_sync` preserves the key, every later sync leaves a confidently-wrong label; if it drops it, the rule was dead code there. That one clause undid all of item 1's honest-unknown work — *absent* reads as unknown and sends the reader to the sha, *stale* reads as authoritative and answers **"do I have the fix?" wrongly**, which is the exact question the field was added to answer, failing hardest on the consumers who sync most often.

Fixed as a **class exclusion, not a named exemption**, so the next derived field cannot land in the same trap:

> **Never-clobber exists to protect values the client cannot REPRODUCE.** A derived field is by definition reproducible, so it is not in the class the rule protects, and applying the rule to it converts *"do not destroy information"* into *"preserve a wrong answer"*.
>
...
`lastSyncedVersion` then falls out automatically and never-clobber applies **solely** to `templateSyncToolsVersion`, for the stated reason rather than by enumeration. **Backstop shipped too**, for when the exclusion is forgotten — round 3's provenance move one level up: *a version label that cannot be checked against the thing it labels is a value that needs provenance*. A new `lastSyncedVersionOf` records the sha the label was computed from, so a reader compares it to `lastSynced`: match means good, mismatch means stale **and known stale**. A confidently wrong answer becomes a detectable one.

**The G5 guard was keyed on provenance again, one layer down.** Its check set excluded *step-4 keep-mine files as a category* — keyed on **which step produced the resolution** when the property that matters is **whether step 2b's specification covers the path**. Those diverge in the direction that loses coverage: a step-4 keep-mine file that is **gitignored** is in 2b's set (a), *is* backed up, would compare fine, and the blanket exclusion dropped it — manifest-tracked, gitignored, keep-mine: no history, no diff, no undo, exactly the class 2b exists for, and **`CLAUDE.local.md` resolved keep-mine is the routine instance, not a corner case.** The set is now `keep-mine ∩ (set a ∪ set b)`, with no step attribution anywhere; step-4 non-gitignored files fall out on their own. And the set is **derived from the specification, never from the backup directory's contents** — otherwise the coverage assertion is vacuous by construction and a 2b derivation failure silently shrinks the set until the guard passes, a guard failing by reporting success while guarding a bug whose signature is failing by reporting success.

**`RUN_GATE_TERMINAL` leaked into the suite, in the worse direction.** `test-hooks.sh` unset `RUN_GATE_ACTIVE` but not the marker path. Under self-gating a fixture's nested recursion guard touches **the real outer run's marker**, and the outer clamp then declines to fire — a genuinely retryable failure reading as terminal, the inverted direction. Latent (the chain must also exit exactly 78), and the same environment leak this release already fixed once. Unset plus a companion pin in the verifier.

**A running session obeys the body it LOADED.** Measured live: an installed `~/.claude/skills/sync-template/SKILL.md` byte-identical to the held branch while sessions started earlier were still executing the v2.2.4 body — so *"re-copy"* and *"drift reports 0"* both pass while every live session runs the old steps. That is this release's own bug moved one layer out: the artifact that runs is not the artifact you verified. Three closes: a **version marker inside the skill body** that step 1 asserts against the installed file and the report states, so a stale session fails loudly on its next invocation; the DoD now says **RESTART-REQUIRED, not re-copy** (the report has told users *"restart before relying on changed agent definitions or skills"* for several versions — the skill knew this about agents and not about itself); and `verify-user-level-drift.sh` now compares against **the last released tag, not the working tree**, because "0 drift" against an unmerged branch certified that an unreviewed, ship-blocked revision had reached a user's machine, which is the opposite of what the probe is for. **State the limit plainly: the marker cannot rescue sessions already at risk** — a session running an *older* body has no assertion in it to fire and can never self-detect. It prevents the next occurrence only.

**The sweep's exit contract is inverted and nothing said so.** `set -o pipefail` is correct, but it leaves `0 = hits found (a problem)`, `1 = clean (success — grep's no-match)`, `2 = malformed invocation`. So `if ! sweep; then fail` marks every clean tree broken, and since the skill mandates putting logic in a script file, `set -euo pipefail` typed from muscle memory **aborts the script on a clean sweep**. The contract is now stated, with *check for 2 specifically; non-zero alone is not an error*, and `pipefail` is **scoped to a subshell** so it cannot change the exit semantics of the hosting script's later pipelines.

**Two comments the mechanism needed.** The provenance marker is a **FILE**, and that is load-bearing on Windows: MSYS rewrites a POSIX-looking env value crossing into a native Windows process (`/tmp/…` → `C:/Users/…/Temp/…`), and since the path comes from `mktemp -d` this happens in the real script — it survives only because both spellings resolve to the same file and the translation is consistent both ways, **never because the value passes through untouched**. Compared as a string it would break silently on Windows and nowhere else. And the marker's honest limit: it proves *a* nested `run-gate.sh` exited terminally during this invocation, **not** that *this* `$GATE_RC` came from it — `bash hooks/run-gate.sh; some-other-tool` sets the marker and then takes its rc from the second command. Contrived, no fix wanted; the comment simply no longer reads as though the marker settles provenance outright.

**Two items closed rather than implemented.** The step-2b `sys.stdout.buffer` report (item L) is **retracted**: it has been prescribed since v2.2.4 and the defect was in the reporter's own scratchpad script, never in the shipped snippet — third instance in one thread of *my instrument's property reported as the system's*. And **K3's second half — "nobody hardcodes a bare `78`" — was NOT satisfied before this round**, contrary to an earlier summary: the value census asserted the two definitions agree, and nothing asserted a third site could not appear as a literal. Checked in the file rather than taken from the summary; the bypass census (verifier check 21c-2e) closes it now.

### 9b. Round 5 — THIRTEENTH fail-open: a config VALUE could kill the hook, and the census that should have caught it reads the wrong surface

**`eval "$TEST_CMD"` ran in the hook's own shell.** `hooks/pre-commit-test.sh` evaluates the consumer's `**Test**` value — or, where `run-gate.sh` is not installed beside it, the raw `**Gate**` value — and a value whose top level reaches `exit` or `exec` therefore terminated **the hook itself**, bypassing the if/else that decides the verdict. Found by independent QA, verified by execution; **pre-existing since v2.1.x, not a regression of this branch.** Measured, before → after:

| `**Test**` value | before | after |
|---|---|---|
| `exit 1` | hook exit **1**, `BLOCKED` lines **0** | exit 2, 1 |
| `exec bash -c "exit 1"` | hook exit **1**, `BLOCKED` lines **0** | exit 2, 1 |
| `exec bash -c "exit 78"` | hook exit **78**, `BLOCKED` lines **0** | exit 2, 1 |
| control `bash -c "exit 1"` | exit 2, 1 | exit 2, 1 |
| control `true` | exit 0, 0 | exit 0, 0 |

A non-2 PreToolUse exit is **warn-and-ALLOW**, so every one of those let the commit through **ungated and silently**, and leaked the captured `$OUT` file. The 78 row additionally violated the invariant this very release documents — *78 is never a hook's own exit status; a hook never forwards a child's exit code*. `exec` is this codebase's own idiom: the R5e fixture wrapper uses it.

**Reachability is consumer-authored only** — no shipped `templates/*/PROJECT_CONTEXT.md` and nothing `setup-project.{sh,ps1}` derives contains that shape. Hence major, not critical. `gate-before-merge.sh` evals no consumer command and is clean (probed: exit 2).

The fix is one pair of parentheses — `( eval "$TEST_CMD" ) > "$OUT" 2>&1`. A subshell contains both builtins that end a process: `exit` ends the subshell, `exec` replaces the subshell's process, and either way `$?` is a child's status reaching the test like any other. **Stated narrowly on purpose:** this is containment of `exit` and `exec`, *not* immunity to arbitrary consumer values, which is not a property an eval boundary can have. Round 4's clamp reasoning is undisturbed — a child's 78 still lands with no provenance marker and still takes the retryable message.

**The durable half is the control gap, and it is worth more than the fix.** Census 21c-2f — *"no `exit 1` in a registered hook"* — was **green throughout**, and it had been *observed* going red when an `exit 1` was appended to the hook's source. It reads **source text**; the `exit 1` here arrived as **config data**. So the claim *"`pre-commit-test.sh` never exits anything but 0 or 2"* was true of the file and false of the process, and the census measured the right property on the wrong surface.

> **Ask of every green number: what would have to be true for this to be green while the thing I care about is broken?** Here the answer was `exec` in a config value.

Two things follow, and both shipped. A **behavioural** counterpart — R5g in `test-hooks.sh` — drives the hook with `exit`- and `exec`-bearing `**Test**` values and asserts exit 2 plus a `BLOCKED` line in each, with a passing-command arm so that "exit 2 on everything" cannot satisfy it. And 21c-2f now **says beside itself** that a source census cannot reach this class, so neither check is deleted on the strength of the other being green.

**There is ONE `eval` but TWO config keys reach it, and round 5's behavioural arm covered one of them (round 6).** Consumer-reported, controller-verified. `pre-commit-test.sh:201` is the only `eval` in the file; `:175` — on the mirror-fallback path taken when `run-gate.sh` is not found beside the hook — assigns the **`**Gate**`** value into `$TEST_CMD` and falls through to that same statement. So the fail-open is reachable through `**Test**` *and* through `**Gate**`, and the second is not a variant: it is the identical statement with a different value source. R5g driving only `**Test**` was therefore a correct behavioural test of *one of the two ways in* — **the finding's own shape one level up.** R5g gains a `**Gate**: exec bash -c "exit 1"` arm with `run-gate.sh` absent beside the hook, asserting the fallback WARN (so the path is *proved* taken, not assumed), exit 2, and a `BLOCKED` line.

**And the Gate path is the LESS VISIBLE of the two.** It prints `WARN: … evaluating the Gate command directly instead` **by design** — so a fail-open there arrives *wearing a warning that looks like the known degradation*. A consumer who sees that line has been told to expect a **lesser** path, not a **bypassed** one, and nothing in the transcript distinguishes them. Reachability was re-swept over **both** keys across all six variants and the root file: every shipped value is either a blanked placeholder or the `dotnet format … && dotnet build … && dotnet test` chain — no `exec`, no top-level `exit`. The major-not-critical rating holds.

> **A census over source text cannot see behaviour that arrives through data.**
>
> Round 5 noted this beside 21c-2f. Round 6 promotes it to a **banner over the whole 21c-\* family**, because `pre-commit-test.sh` will not be the last hook to eval a consumer-authored string and the source census will be green for every one of them. The banner states the rule, names 21c-2f as the worked example, records that round 6 found a *second* key reaching the same eval, and says that an eval boundary needs a behavioural counterpart covering **every** key that reaches it — not just the obvious one.

**The eval boundary's missing terminal arm is now mechanical, not a comment (item 1).** Round 4 correctly declined to clamp there — with no terminal arm present the clamp was unobservable, and shipping it would have been the decorative-guard shape this release forbids. But that safety is a fact about *today*, and **a comment defends against a reader, not against a refactor: the person who adds a terminal arm is precisely the person not reading the note about hypothetical terminal arms.** Verifier check **21c-2h** asserts the **negative** — nothing below the `eval` keys on `GC_TERMINAL_RC` — and its failure message names the *prerequisite* (give the boundary a provenance channel first) rather than the symptom. It is positional by necessity, like 21c-2d: the identical test one branch up is the `run-gate.sh` path and is legitimate, so a whole-file grep would be red on day one. It duplicates no coverage: this file is the toolkit's per-commit `**Test**`, `test-hooks.sh` runs only under the full `**Gate**`.

**Propagation gating now ASSERTS what makes it valid — as two assertions, because verbatim-install is not the one that matters (item 3).** A consumer flagged the hazard from their own false positive: for a placeholder-bearing file the released reference is the manifest's **processed** hash, not the tag's raw blob, and their `pre-commit-test.sh` read as drift against the blob (`templateRawHash 07de572b…`) while matching the processed template exactly (`templateHash cd29e1b1…`). A check that noisy is disabled within a week, which is how a real check dies.

Blob-versus-tag is nevertheless correct here — but **not because placeholders are absent**. Three in-scope files carry them (`hooks/lib/git-cmd.sh` 2, `hooks/pre-commit-test.sh` 1, `skills/sync-template/SKILL.md` 3) and the live copies carry them identically. It is correct because **the user-level install is VERBATIM**: there is no substitution step at user level, unlike the project bootstrap. The files match precisely *because* nobody processes them.

That property explains why the comparison is **valid**. It says nothing about whether the files **work**, and the two come apart exactly where it matters:

> **A placeholder added in a shell VALUE position keeps verbatim-install true, keeps the blob comparison green, and makes the hook execute a literal `{{...}}`.** The new assertion passes, drift reports 0, and the hook is broken.

That is v2.2.0's `- **Protected branches**: {{DEFAULT_BRANCH}}` — a config value read as data — one directory over. So both ship, labelled by what each protects:

| assertion | protects | where | red means |
|---|---|---|---|
| verbatim install | the drift check's **validity** | `verify-user-level-drift.sh` | blob-versus-tag is no longer a legitimate baseline; use the manifest's processed hash |
| no placeholder in an **executable position** | the hooks' **correctness** | verifier check 21c-3b | a shipped user-level hook will run a literal `{{...}}` |

**Only the second catches the failure worth catching**, and it is pinned to a measured number rather than a hoped-for one. **Use the refined sweep arms or the assertion is red on day one, permanently, and gets disabled by someone whose reasoning looks sound**: a plain `grep -rn '{{[A-Z_]\{2,\}}}' user-level-reference` returns **29** hits, not one of them actionable — `.mcp.json.template` 12 (a never-installed template whose placeholders are *supposed* to be unfilled), `README.md` 8, `sync-template/SKILL.md` 6, and 3 comment lines across the two hooks. The `SKILL.md` set includes the passage documenting this exact false positive: *the detector flags the documentation of its own false positives.* With the refined arms (shell comment lines excluded, markdown hits required on a `- **Key**: value` line) both arms measure **0**.

**The scanned surface is `*.sh` + `*.md` only, said out loud because a bare 0 is otherwise overread.** `settings.json` is not scanned, and a settings.json hook `command` *is* an executable position — the same class as the `{{DEFAULT_BRANCH}}` case above. Measured: no `*.json` under `user-level-reference/` carries a placeholder at all except `.mcp.json.template`, which is never installed. The boundary is real; nothing sits outside it today. The check is **born with its control in band**: every run first drives the detector against a planted fixture — a placeholder in a shell value line must be found, the same placeholder in a BOM'd comment must not, a markdown config line must be found, inline prose must not — and if that self-test fails the 0 is reported as INERT rather than as a pass.

**The verbatim assertion reproduced the consumer's own lesson while being written.** Its first cut counted placeholders in the **working tree**, which reported `SKILL.md` as PROCESSED (11 reference placeholders against 4 live) on a live tree that was byte-identical to the released tag. Wrong baseline, not processing — the identical mistake, one directory over, inside the check added to prevent it. It now counts against the same reference `check_file` uses, falling back to the working tree only for files absent from the tag, which is its one **independent** red: an unreleased reference file is skipped by every other line in the script.

**And its honest limit, stated rather than glossed:** for a file that *is* compared, a processed live copy differs byte-wise too, so `DRIFT` fires as well — there the verbatim assertion contributes the **cause**, not the red.

**`verify-user-level-drift.sh` now states the `settings.json` exclusion and its reason (item 2).** The loop covers `CLAUDE.md` + `hooks/` + `skills/` + `agents/`; `user-level-reference/settings.json` ships as reference material and is **never compared**. That is deliberate — a user's own `~/.claude/settings.json` legitimately carries their personal permissions, env and MCP entries, so force-matching it would report every user as drifting from a file they were never meant to copy verbatim — but nothing said so, and its presence in the reference tree implies it is tracked. Same class as `.mcp.json.template`, `README.md` and `settings-reference.md`, which a consumer's audit flagged as MISSING and which are never installed by design. The header now says so, with a *do not add these to the loop* line, and records that extending the script to any **processed** tree requires the manifest's processed hash instead.

(Scope note for the bare-`78` census added in round 4: it scans `hooks/*.sh hooks/lib/*.sh` only.)

**Round 6's measured JSON residual is CLOSED, not re-dated — at BOTH ends (round 7).** 21c-3b scans `*.sh` + `*.md` only, so JSON — where a hook `command` string lives, an **executable position**, the same class as v2.2.0's `{{DEFAULT_BRANCH}}` config value read as data — had **no placeholder detector at either end**: not in the census, and not in the skill's consumer-side sweep (its markdown arm needs a `- **Key**: value` line, its shell arm is `--include='*.sh'`). A literal `{{...}}` in a `command` string is a path that does not resolve → `127` → fail-open, and that is this release's own class: `templates/general/.claude/settings.json` carries **30** `command` strings of which **9** mention a 127 guard — byte-identical on a live consumer, so the ratio is shipped rather than local. Not 21 live holes; the point is that the surface no sweep covered is the surface where a missing path fails open *by default*.

**And it was the cheapest arm to add, not the most expensive.** The `*.sh` and `*.md` arms needed refinement — comment-line exclusion, the `- **Key**: value` shape — precisely because those formats carry prose that *legitimately* mentions placeholders. **JSON has no comments, so any `{{...}}` in a `.json` file is in a value by construction:** no exclusions, no shape requirement, no false-positive class to tune. Both ends ship pinned at the measured **0**:

- `21c-3c` — repo-side census over `templates/**/*.json` + `user-level-reference/**/*.json`, the same command round 6 ran by hand.
- the sync-template sweep's **json arm** — `git grep -n -e '…' -- '*.json'`, tracked files only.

**JSON needs no content-shape tuning; it DOES need scope discipline — two different things.** A `find ~/.claude -name '*.json'` arm was measured red on its first run on a real machine: **3430 files, 1 hit**, in a dated backup nothing reads, carrying `{{GUIDE_TEMPLATE}}`/`{{USAGE_DATA}}`/`{{WINDOW_DAYS}}` — *another tool's* keys. That is the "reports a problem forever, gets disabled within a week" failure reappearing inside the arm designed from the argument against it. Both ends scope to what the toolkit **owns** — never a filesystem walk, and explicitly **not** by filtering on known toolkit key names, which rots on the first new key and would hide a genuine placeholder under it. The user-level half is a repo-side census that never touches `$HOME`: the only user-level JSON shipped is `user-level-reference/settings.json`, one file, which `verify-user-level-drift.sh` deliberately excludes as user-owned anyway.

**Two structural arguments are written into the code, because a future cleanup would otherwise merge or drop one of them.** First: the repo-side census is sufficient at user level **only because the install is verbatim** — reference clean + verbatim install ⇒ live copy clean. That verbatim property is round 5's separate assertion in `verify-user-level-drift.sh`; the two look independent and are **one argument with two halves**, so each now names the other. Second, cutting the *opposite* way: for the project tree the census is **necessary but not sufficient**, because the project install is not verbatim — `template_apply_file` substitutes only placeholders present in **the project's** manifest, so a key the manifest predates lands as a literal. **That is the v2.2.0 chain exactly: template correct → substitution incomplete → consumer carries `- **Protected branches**: {{DEFAULT_BRANCH}}` → trunk unprotected, with a template-side census green throughout.** An install defect, not a shipping defect. The skill's json arm is the only detector for that class, and `21c-3d` pins its presence so a tidy-up cannot delete it as redundant.

**`git grep`, not `git ls-files … | xargs grep` — measured.** `xargs` maps any grep exit in 1–125 to its own **123**, so a clean sweep and a malformed invocation become the same code, collapsing the distinction round 4's `pipefail` work exists to expose. `git grep` needs no pipeline and keeps 0 = hits / 1 = clean; it reports malformed as **128** rather than grep's 2, so the sweep's exit contract now states the testable form — *0 means hits, 1 means clean, anything else means broken* — with the literal `2` marked grep-specific.

**Controls, with the negative doing the real work.** Positive: a planted `{{…}}` in a JSON value inside the scoped set → red. **Negative: the same plant outside the scoped set → green** — without it the *scoping* is untested and the next person widens it back to a walk. A third fixture pins the `.mcp.json.template` extension carve-out. Delete-the-guard, unconfounded (planted in all six variants so the cross-variant identity check stays green): the suite reports **six shipped placeholders as its only FAIL**, and with the arm removed goes fully green with all six still shipped. For the consumer arm, in a scratch git repo: markdown and shell arms both exit 1 (clean) on a `.claude/settings.json` carrying a placeholder; the json arm exits 0 and names it; the *untracked* copy of the same file is invisible to the json arm and would be flagged by a filesystem walk.

**Stale counts removed rather than updated (round 6).** Root `CLAUDE.md` and the self-gating census comment in `verify-template-consistency.sh` both carried `342/0 standalone versus 323/19 under the gate` — two releases out of date. Per this release line's own v2.2.3 precedent, **prose that names no number cannot go stale**, whereas a second place holding a count drifts out of agreement with the authoritative one. Both now describe the *effect* (the suite unsets an inherited `RUN_GATE_ACTIVE`; every fixture nesting `run-gate.sh` would otherwise trip the recursion guard and skip) and point at this file, which is the one place that carries the counts.

### 10. Verification

`verify-template-consistency.sh` **283/283** (+19 over v2.2.4: three self-gating assertions, the `RUN_GATE_ACTIVE` pin, the `GC_TERMINAL_RC` census, the sweep-ordering pin, the matrix script joining the shell-syntax sweep, round 4's six — the `RUN_GATE_TERMINAL` pin, the guard-ordering census, the bare-`78` bypass census, the no-`exit 1` census and the two SKILL-marker censuses — round 5's three: 21c-2h's no-terminal-arm census and 21c-3b's two, its in-band detector self-test and the pinned zero — and round 7's three: 21c-3c's in-band detector self-test and its pinned JSON zero, plus 21c-3d's consumer-arm pin). `test-hooks.sh` **376 assertions** (+34 over v2.2.4: the terminal-vs-retryable arms, the clamp control, the second terminal guard, round 4's eight — R5e's wrapper-nested marker and R5f's two remaining 78-bearing paths — round 5's eight, R5g's three `exit`/`exec` config-value arms with their message assertions, the 78-framing arm and the passing-command control, and round 6's two: R5g's `**Gate**`-through-the-mirror-fallback arm, asserting the fallback WARN and the `BLOCKED` line), all three parser configurations, with the interpreter each one resolved to printed by the harness:
...
**Every guard in this release ships with a control, and every control was verified BY DELETING ITS GUARD.** This supersedes "defend it with a comment": a comment defends a guard against a reader, not against a refactor, a merge, or a tidy-up six months out — and a guard whose non-firing is indistinguishable from its absence gives a maintainer no signal at all when they remove it. That is the same detectability problem the guards themselves address, one level up.

> **A guard that cannot be observed firing is indistinguishable from one that was deleted — so every guard against an invisible failure ships with a control that makes it visible.**

The audit needs a guard of its own, because a control written in a hurry passes for the wrong reason: assert "the guard fired" by observing a nonzero exit and the control also passes when the command failed for something unrelated, **or when the guard is gone entirely** — leaving a green control, a deleted guard, and a *stronger* false sense of coverage than before the audit. Reading a control tells you it looks fine; deleting its guard tells you the truth. And a control **set** needs both arms: positives alone cannot distinguish a working guard from one that fires on everything — the BOM sweep's fourth control (a BOM'd comment that must *not* fire) was not thoroughness, since three positives alone would pass against a regex matching everything.

Results, all red where they must be:

| guard | control | with the guard deleted |
|---|---|---|
| sweep option ordering | verifier check 21c-2c | `total=1 bad=1` — red |
| `GC_TERMINAL_RC` census (name **and** value) | verifier check 21c-2b | one copy set to 79 → `found=1 of 2` — red |
...
| **round 5:** no terminal arm at the `eval` boundary, *mechanically* | verifier 21c-2h (new) | guards a NEGATIVE, so the control ADDS what must stay absent: a terminal arm spliced above the `BLOCKED` line → red, naming the provenance prerequisite. Baseline green |
| the `( eval … )` subshell | R5g (extended in round 6) | the parentheses removed → **9 of 10** flip (`exit 1` → hook exit 1, `exec … exit 78` → hook exit 78, `**Gate**: exec …` → hook exit 1 *with the fallback WARN still printed*, zero `BLOCKED` lines throughout). The tenth is the passing-command arm and correctly stays green — so the block is not one that fires on everything. The Gate row is the one worth reading twice: the guard removed, that arm fails while the transcript still shows the by-design `WARN: … evaluating the Gate command directly instead`, which is exactly why the bypass is invisible there |
| no placeholder in an executable position | verifier 21c-3b (new) | four arms: a placeholder in a shell VALUE line → red; **the same placeholder in a BOM'd comment → green**; a markdown `- **Key**: value` line → red; the detector's own regex broken → **INERT**, not a silent 0 |
| **round 7:** no placeholder in a shipped `*.json` | verifier 21c-3c (new) | three arms: a placeholder in a JSON **value** inside the scoped set → red; **the same placeholder OUTSIDE the scoped set → green**, which is what makes the *scoping* testable rather than assumed; a `.mcp.json.template` sibling → green, pinning the extension carve-out. Delete-the-guard, planted in all six variants so the cross-variant identity check stays green: the census is the suite's **only** FAIL, and with the arm removed the suite is fully green with six placeholders shipped |
| the sweep's own json arm | run in a scratch git repo | `.claude/settings.json` carrying a placeholder: markdown and shell arms both exit **1** (clean), the json arm exits **0** and names the file → **it is the only detector**. Negative: an *untracked* copy of the same file is invisible to `git grep` and *would* be flagged by a filesystem walk, so the tracked-files scoping is tested rather than assumed |
| the *pin* on that arm | verifier 21c-3d (new) | the `git grep … '*.json'` line deleted from `SKILL.md` → 21c-3d red. The two other censuses over the same file — 21c-2c (sweep ordering) and 21c-3 (BOM tolerance) — both stay **green** on the same mutation, which is what makes 21c-3d non-redundant rather than a third grep of a file already grepped twice |
| verbatim install | the drift script's own fixture (a fake `HOME`; nothing written to the real `~/.claude`) | placeholders substituted in the fake live copy → `VERBATIM INSTALL VIOLATED` + the `PROCESSED` line; guard deleted → the `PROCESSED` line disappears while `1 drift` remains, which is the honest limit recorded above: for a *released* file this assertion contributes the cause, not the red |
...
**The matrix found a latent defect in the suite on its first honest run.** It is the first thing to run `test-hooks.sh` with `python3` **genuinely off PATH** rather than merely non-functional, and `mkpathdir` was building the python3-only stub directory unconditionally — reporting `FAIL stub PATH minimum tool set (missing: python3)` in a configuration that was otherwise skipping correctly (`238 passed, 1 failed, 119 skipped`). That report is a *loud* guard and right for **core** tools, whose silent absence corrupts every case run under a stub PATH; `python3` and `jq` are not core, they are the tools being varied, and their absence is already handled by the `HAVE_*` skips. The harness was failing for a harness reason and reporting it as a hook result — the exact shape this release exists to stop. The two optional stub directories are now built only where their parser exists; no assertion changed and the skip counts are untouched.

The three-configuration run is now a **script** — `scripts/test-hooks-parser-matrix.sh` — rather than a remembered procedure, because the remembered version was performed wrongly and the wrong result looked right. It builds every restricted PATH in MSYS spelling only, prints what `node`, `python3` and `jq` actually resolve to, refuses to run a configuration whose hidden interpreter is still visible, and **asserts the skip count is non-zero and in band**. A restricted configuration reporting zero skips is proof the restriction did not apply, not proof of improvement.
...
- **Item 7 also changes two exit codes.** `hooks/run-gate.sh`'s self-reference guard exits **78** (was 2), and its *"not inside a git repository"* guard exits **78** (was 1). If any of your tooling keys on either of those, or treats "nonzero == 1", update it. Ordinary red gates still exit 1, `run-gate.sh`'s own `cd "$REPO_TOP"` failure still exits 1 (it is a runner, and a `cd` failing on a just-resolved path is a transient fault), **`pre-commit-test.sh`'s two `cd` failures now exit 2** (round 4 — from a hook, `exit 1` is warn-and-ALLOW and let the commit through ungated), and **a gate command of your own that exits 78 is clamped to 1** — so a consumer gate legitimately using `EX_CONFIG` keeps the ordinary retry advice rather than inheriting the terminal remedy. If you have a terminal guard of your own inside your gate chain, move it to 78 **after** you adopt this release's `pre-commit-test.sh`, not before: ahead of that it is just a stranger nonzero and behaves worse than the 1 it replaces.
- **The delivery check for this release is a PROBE, not a step you tick — and the definition of done is RESTART-REQUIRED, not re-copy.** `scripts/verify-user-level-drift.sh` compares `user-level-reference/{hooks,skills,agents}` against your live `~/.claude/` tree file by file, and `user-level-reference/skills/sync-template/SKILL.md` is in that set. **It now compares against the last released tag** rather than the working tree (round 4): "0 drift" against an unmerged branch is not evidence of delivery, it is evidence an unreviewed revision reached a machine. Files that exist only on a branch are listed as `UNRELEASED`. Pass `--worktree` for pre-release inspection; that mode prints a banner and is never a release check.
- **Copying the file is not the end of the migration.** A session that has already loaded a skill keeps running the body it read — measured: an installed `SKILL.md` identical to the new release while live sessions still executed the previous version's steps, with the drift check reporting clean the whole time. **Restart your session** after installing a changed user-level skill or agent. `SKILL.md` now carries a `SYNC-TEMPLATE-SKILL-VERSION` marker that step 1 checks against the installed file, so the *next* stale session fails loudly — it cannot help a session already running an older body, which has no such check in it to fire.
- **Item 10 — if you spliced anything into `run-gate.sh` between `echo "GATE: running: $GATE_CMD"` and `cd "$REPO_TOP" || exit 1`, hand-merge that region this round.** That window is the last point before the gate runs and therefore exactly where consumers splice (one reporting consumer's dependency preflight lives there). v2.2.5 now occupies it with the five-line *"this `exit 1` DELIBERATELY STAYS 1"* comment. One consumer's merge script asserts `tl[te+1].startswith('cd "$REPO_TOP"')` and **fails against v2.2.5 — correctly**: it is refusing to reassemble their file around a changed template shape, their own guard earning its place on a change neither side anticipated. When you re-merge, **the new comment must travel with the `cd`**, not be stranded above your splice; the failure mode if unnoticed is a comment attached to the wrong statement — cosmetic now, misleading later.
- **Item 9b — RE-COPY `hooks/pre-commit-test.sh` into every repo that carries it, and into `~/.claude/hooks/` if you keep a copy there.** The one-line subshell fix is the thirteenth fail-open and the only round-5 change with consumer-visible behaviour. You are exposed **only if your `**Test**` or `**Gate**` value reaches `exit` or `exec` at its top level** — nothing shipped does, so most repos are fine — but if yours does, your commit gate has been silently passing every commit. Quick self-check without adopting anything: run your own `**Test**` value as `bash -c '<value>'` and then as `( <value> )`; if the first ends your shell, you were exposed. Nothing else about the hook's contract changed, and a repo whose value is an ordinary command sees no difference at all.
