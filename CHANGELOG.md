...
**One measured residual recorded beside 21c-3b, rather than left silent (round 6).** That census scans `*.sh` + `*.md` only, so `settings.json` — where a hook `command` string lives, an **executable position**, the same class as v2.2.0's `{{DEFAULT_BRANCH}}` config value read as data — sits outside its arms. Swept by hand: `grep -rn --include='*.json' -- '{{[A-Z_]\{2,\}}}' templates user-level-reference` returns **0**, across both trees (the glob excludes `.mcp.json.template` by extension, the same carve-out the arms make and for the same reason — it is never installed). The boundary is real; nothing sits outside it today. It is now written down **with the date and the measurement**, as a gap that has been looked at and found empty rather than one nobody noticed, and with the instruction to widen the arms — not the interpretation of the number — if that stops being true.
