...
### Added
- New consistency check 50 asserts every `mcp__<alias>__<tool>` token in a `templates/**/agents/*.md` or `user-level-reference/agents/*.md` `tools:` line names a tool the alias's OWN mcp-dev-servers module actually exports, via two independent censuses run by new `scripts/lib/list-mcp-tools.py` (system `python`, never `server/.venv`): a static scan of the source tree and, for a registered alias, an import census (`asyncio.run(mcp.list_tools())`) in mcp-dev-servers' own venv. Disagreement between the two, or an allowlisted tool missing from the static census, is red; an unregistered family alias (`git-tools`, `github-tools`, `dotnet-tools`, `ollama-tools`, `rust-tools`, `python-tools`) is an in-band SKIP (static census only), and the whole check is a single SKIP naming `MCP_DEV_SERVERS_DIR` when no mcp-dev-servers checkout can be found (env override or derived from a registered family alias's venv path in `~/.claude.json` — never a hardcoded path). Two-sided via 50c: a scratch copy of one server module with one tool function renamed is asserted to be flagged by the same static scanner and comparison the real check uses (item 23; item 24, a version-floor alternative, was dropped as measured-unworkable — see the design spec).
