...
Yutraffic-Challenge re-synced to confirm v2.1.1/v2.1.2 hold (apply order, hot-reloaded `${CLAUDE_PROJECT_DIR:-.}` paths, region markers, a clean manifest drop) and flagged a latency gap: `hooks/pre-commit-test.sh` re-derives and re-evals the Test/Gate command from `PROJECT_CONTEXT.md` on every commit, duplicating work `hooks/run-gate.sh` already does — and a green commit gate leaves nothing behind for `gate-before-merge.sh` to check at merge time, forcing a second ~2-minute gate run there. `pre-commit-test.sh` now prefers `run-gate.sh` when it sits alongside the hook (it always does, once synced) and the project declares a `**Gate**` command: it shells out to `run-gate.sh` instead of evaluating the command itself, so a green commit writes `.gate/last-pass.json` as a side effect and `run-gate.sh` becomes the single source of truth for both commit- and merge-time gating. A project with only a `**Test**` field, or without `run-gate.sh` present, keeps the prior behavior unchanged. Mirrored to `user-level-reference/hooks/pre-commit-test.sh`; 5 new fixtures in `scripts/test-hooks.sh` cover the pass/fail/absent-run-gate.sh cases (165 total, up from 160).

### Downstream migration

1. `/sync-template` will pick up the edit to `.claude/agents/architect.md` as an AUTO_UPDATE, unless your project has locally edited that file — in which case it shows as CONFLICT and any project-specific customizations should live inside the file's PROJECT-CUSTOM region, not mixed into the template body.
