...
| the `( eval … )` subshell | R5g (new) | the parentheses removed → **7 of 8** flip (`exit 1` → hook exit 1, `exec … exit 78` → hook exit 78, zero `BLOCKED` lines). The eighth is the passing-command arm and correctly stays green — so the block is not one that fires on everything |
