...
12 hook scripts (2 lib files), 8 skills; 212 → 230 consistency assertions and 204 → 261 hook fixtures. The node-only, python3-only and jq-only cases SKIP where that interpreter is absent (the suite prints a `skipped: N` tally, counted per assertion), so the fixture total is host-dependent: 261 is the count with all three present.
