...
- After syncing, `template_verify` (item 22) is expected to read all-green (`unknown_keys_empty` PASS, `superseded_absent` PASS). Before syncing on v4.0.1, `unknown_keys_empty` FAILs by design — that is the release's own "who still needs to sync" signal, not a defect. The PO can run `bash scripts/verify-consumers.sh <dir>…` across the known consumer checkouts to see the fleet's status at a glance.
- A consumer bootstrapped with `setup-project.ps1` before this release carries a `template-manifest.json` with CRLF line endings and no trailing newline, so `template_verify`'s `manifest_bytes` line FAILs on their first v4.0.1 verify — same shape as the `unknown_keys_empty` clause above: an expected, self-identifying pending-sync signal, not a defect to fix by hand. It self-heals: `template_finalize_sync` rewrites the manifest LF-only with exactly one trailing newline, so one `/sync-template` run clears it.
...
- Gate artifacts now live under `.git/gate/` (shared by every worktree); the old `.gate/` directory in a checkout can be deleted. A merge right after upgrading may need one fresh `bash hooks/run-gate.sh` — the old per-checkout artifact is read once more as a fallback, but the new shared location has nothing until then.
