...
- **`templates/*/CLAUDE.md` PROJECT-CUSTOM BEGIN comment** now reads "...routing blocks (context-mode, …), and extensions go here." — credit penumbra. The context-mode plugin's `writeRoutingInstructions()` re-appends its block whenever `CLAUDE.md` lacks the literal substring `context-mode`; naming it in the placeholder comment is a cheap way to make an otherwise-empty PROJECT-CUSTOM region opt the file out of that re-append, without touching the marker lines the consistency check greps for (`PROJECT-CUSTOM:BEGIN`/`PROJECT-CUSTOM:END`, both unchanged).
...
5. Re-run `bash scripts/verify-user-level-drift.sh` from anywhere in the repo (it now `cd`s to root itself) to see the widened `hooks/`/`skills/`/`agents/` comparison against `~/.claude/`.
6. `CLAUDE.md`'s PROJECT-CUSTOM comment text will show as an ordinary diff on the next sync — accept it; it does not touch the marker lines your custom content lives between.
