...
3. If your project's `.gitignore` does not ignore `.gate/`, add it — the working-tree key would otherwise fold the previous run's artifact into the next run's hash.
4. Nothing else changed — no settings.json delta, no new files.

12 hook scripts, 8 skills; 212 consistency assertions (unchanged) and 184 → 202 hook fixtures.
