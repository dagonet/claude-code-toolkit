...
- **Item 7 also changes two exit codes.** `hooks/run-gate.sh`'s self-reference guard exits **78** (was 2), and its *"not inside a git repository"* guard exits **78** (was 1). If any of your tooling keys on either of those, or treats "nonzero == 1", update it. Ordinary red gates still exit 1, `cd` failures still exit 1, and **a gate command of your own that exits 78 is clamped to 1** — so a consumer gate legitimately using `EX_CONFIG` keeps the ordinary retry advice rather than inheriting the terminal remedy. If you have a terminal guard of your own inside your gate chain, move it to 78 **after** you adopt this release's `pre-commit-test.sh`, not before: ahead of that it is just a stranger nonzero and behaves worse than the 1 it replaces.
- **The delivery check for this release is a PROBE, not a step you tick.** `scripts/verify-user-level-drift.sh` compares `user-level-reference/{hooks,skills,agents}` against your live `~/.claude/` tree file by file, and `user-level-reference/skills/sync-template/SKILL.md` is in that set. So the definition of done is *"the drift script reports 0 drift"*, not *"remember to re-copy SKILL.md"* — the same move as the rest of this release, replacing a remembered instruction with a measured one. Run it after pulling; anything it lists is a file this release changed and your live copy has not received.
