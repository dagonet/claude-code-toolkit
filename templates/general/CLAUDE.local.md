...
- **`ollama-tools`** — local LLM preprocessing: `local_first_pass`, `extract_json`, `map_project_structure`, plus health/model mgmt (`ollama_health`, `ollama_list_models`, `warm_models`).
...
## SQLite Database Queries -- SHOULD

Claude SHOULD prefer the SQLite MCP (`mcp__sqlite__`) over Bash `sqlite3` commands.
