...
When fixing bugs, trace the ENTIRE data flow through all layers (Repository -> Service -> ViewModel and back) before implementing a fix. Do not fix only the first issue found -- check all layers. Common miss: fixing the read path but not the write path, or vice versa.
