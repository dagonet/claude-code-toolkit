...
7. **Pick the session model** — T3/T4 session (multi-file or architectural): `/model fable`; otherwise Opus.
...
|---|---|---|
| **Java backend** | `java-coder` | Services, models, controllers, `pom.xml`/`build.gradle`, Spring config |
| **Frontend** | `coder` | Components, stores, TypeScript, CSS |
| **Mixed/General** | `coder` | Cross-cutting features or unclear domain |

**Agent fallback:** The `java-coder` agent uses Bash `mvn`/`gradle` commands for build and test (no Java-specific MCP tools exist yet). Do NOT substitute `coder` for `java-coder` — it contains Java/Spring-specific knowledge (DI patterns, JPA conventions, project structure) beyond build tool usage.
...
Conventions: see `.claude/rules/java.md` (loads when you touch matching files).
...
Project-specific reminder: trace read **and** write paths through Controller → Service → Repository → Database — a common miss is fixing one direction but not the other.
