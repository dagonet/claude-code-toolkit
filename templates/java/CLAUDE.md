...
- **Maven**: `mvn test -pl module -Dtest=ClassName`
- **Gradle**: `./gradlew :module:test --tests ClassName`
...
When fixing bugs, trace the ENTIRE data flow through all layers (Controller -> Service -> Repository -> Database and back) before implementing a fix. Do not fix only the first issue found -- check all layers. Common miss: fixing the read path but not the write path, or vice versa.
