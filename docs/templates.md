...
| `CLAUDE.local.md` | MCP usage rules (gitignored, contains machine-specific paths). **Still shipped in v3.1**: its planned retirement was deferred to v3.2 because it is loaded at session start, and the `paths:`-scoped rules files that would have replaced it are not — replacing an always-on channel with one that never arrives at spawn is a downgrade, not a cleanup |
