...
| `.claude/rules/` | Path-scoped language conventions (v2.0 PR4). Each `*.md` carries a `paths:` frontmatter glob list and loads **only** when Claude reads or edits a matching file. **Corrected in v3.1, on measurement:** a rule without `paths:` is **delivered to nobody** — not, as previously stated here, "loaded at launch at CLAUDE.md cost". No rules content of any kind is in a subagent's context at spawn, even when scoped. The exception is `project.md`, seeded by v3.1 as a project-owned `once` file and deliberately unscoped: it is a place for a consumer to add their own `paths:`-scoped conventions, and its own header says so. `general` ships none (no language of its own); `dotnet` → `csharp.md`, `dotnet-maui` → `csharp.md` + `xaml.md`, `rust-tauri` → `rust.md` + `frontend.md`, `java` → `java.md`, `python` → `python.md` |
