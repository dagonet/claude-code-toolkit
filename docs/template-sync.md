...
## Manifest v3: `.claude/template-manifest.json`

Generated by `setup-project.ps1` (Windows) or `setup-project.sh` (Linux/macOS); v2 manifests are migrated by `template_migrate_manifest` (skill step 1c). The v3 format (`manifest_version: 3`, since toolkit v3.1 / server 0.3.0) tracks:

| Field | Purpose |
|-------|---------|
| `manifest_version` | Schema version, currently `3` (`verify` line `manifest_version_3` FAILs on anything else) |
...
| `requires_server` | Server floor, e.g. `">=0.3.2"` — enforced at `template_load_manifest` on every sync after the first |
| `placeholders` | Concrete values used during setup (for reverse-mapping) |
| `files` | One entry per tracked path: `{"ownership": "template", "hash": "sha256:<hex>"}` for template-class files (the hash is of the placeholder-rendered content the server wrote), `{"ownership": "once"}` for once-class files; project-class files have no entry |
