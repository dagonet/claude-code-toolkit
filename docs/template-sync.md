...
**Top-level key order is part of the shape.** The table above is also the order the writers emit:

```
manifest_version, variant, templateRepo, template_version, template_commit,
requires_server, placeholders, files[, deletedAcknowledged]
```

`/sync-template` rebuilds the mapping in that order rather than assigning into the loaded dict — assignment appends, and because every writer mutates-and-re-dumps, an appended key stays appended forever, past the whole `files` map where a reader checking the header never sees it. Every other check reads BY KEY and is blind to position; unknown top-level keys are preserved at the end rather than dropped.
