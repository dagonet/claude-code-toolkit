...
**The 31 lines, in order (v4.1.0 — 24 through `tree_clean` below, plus six new CLAUDE.md/grants lines after `region_markers`; R-A. v4.1.1 adds `project_md_seed_differs`, directly after `project_md_seed_current`):**
...
| `project_md_seed_current` | INFO | never — reports whether `.claude/rules/project.md` still carries the pre-v4.0.1 seed's false "delivered to nobody" sentence (v4.0.1), predates the v4.0.2 "picked up at the NEXT session start" sentence with neither the false sentence nor `paths:` frontmatter, predates the v4.1.0 rewording that points at `.claude/project-instructions.md` instead of `CLAUDE.md`'s retired region (R-G), still names the retired region by its marker `PROJECT-CUSTOM` (v4.1.1, a third harm-keyed arm — `measured: ".claude/rules/project.md header references PROJECT-CUSTOM, a region v4.1.0 removed"`, `remedy: "hand-edit .claude/rules/project.md (once-class: the sync never writes it): repoint the sentence at .claude/project-instructions.md"`), is current, or is `paths:`-scoped (the seed sentences are about being unscoped, so a scoped file is exempt from this line by its own edit — see `project_md_scoped_consistent`). Every un-migrated (v2 or v3) consumer reads older here by construction until they hand-edit the file — expected, not a defect. v4.0.3 (items 9, 11): both pre-v4.1.1 arms' remedy says the consumer HAND-EDITS this once-class file (the sync never writes it), names the sentence to add ("A new or edited rules file is picked up at the NEXT session start, not the current one"), and — for a consumer migrated before v4.0.1 whose header may still carry migration hunks — tells them to move those hunks into the region or a scoped rules file FIRST, before replacing the header |
| `project_md_seed_differs` (v4.1.1) | INFO | never — a FACT line, not a remedy: deliberately not a byte-compare (that would tell a consumer who edited the file on purpose to discard the edit, and produces no surgical remedy either way). Reports whether `.claude/rules/project.md`'s content matches the shipped seed at this template: not present (`measured: ".claude/rules/project.md not present"`, no remedy); shipped seed unreadable (`measured: "shipped seed not found at {shipped_seed_path} -- cannot compare"`, no remedy); prefix holds — consumer appended rules after the shipped seed text (`measured: "seed matches the shipped seed (rules appended after it)"`, no remedy); prefix fails — consumer edited the seed text itself (`measured: "seed text differs from the shipped seed at this template"`, `remedy: "shipped seed: templates/<variant>/.claude/rules/project.md; this file is once-class and no sync writes it"` — the `<variant>` token is literal, never substituted) |
