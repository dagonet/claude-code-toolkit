...
A verification done on A transfers to B only when every byte that differs is accounted for, and the instrument depends on the merge shape. For a fast-forward or added commits, assert ancestry (`git merge-base --is-ancestor A B`). For a squash merge — the normal release shape; the v3.0.4 release commit `1450034` is intentionally NOT a descendant of the verified tip `aae6cf8` — assert tree equality of every path the verdict rests on instead (`git diff --stat A B -- hooks scripts templates user-level-reference` empty, plus the fixture bytes), and account for any path that differs (here: VERSION line 2 only). A non-descendant with equal hashes means someone reconstructed the tree: fine when it is a squash you can explain, disqualifying when it is not. The question is never "is it a descendant" but "can I account for the diff". The earlier wording ("descendant plus byte identity") would have rejected every squash-merged release, including v3.0.3 and v3.0.4 themselves. Hook-hash identity carries hooks; it does not carry SKILL.md or `scripts/test-hooks.sh`, which are executed procedure and is re-read; a transfer claim says which items transferred by hash and which were re-checked (panoscribe). Pair with the rule (yutraffic): the crisp version of a rule is usually the one that has not met the second case yet. "Docs-only" is a description of intent until the diff confirms it.
...
#### Skill step 6a stamps `deletedAcknowledged`, which the server never reads (MM-Agent measured, mcp-dev-servers answered)
After MM-Agent stamped `deletedAcknowledged: [requirements-engineer.md, test-writer.md]` for two TEMPLATE_DELETED agents kept for their PROJECT-CUSTOM content, `template_compute_status` still reported `template_deleted 2` and listed both. The server owner confirmed: `deletedAcknowledged` has zero references in mcp-dev-servers — it is skill-text only, and `template_compute_status` iterates every `manifest["files"]` entry and reports TEMPLATE_DELETED whenever the template no longer ships the file, regardless of any other manifest key. The server-side contract already exists: `template_finalize_sync(deleted_files=["requirements-engineer.md","test-writer.md"])` drops those manifest entries even though the project keeps the files, which become untracked project-owned files; entries are also auto-dropped when the template stopped shipping the file AND the project removed it. Decision (controller): no server change — one mechanism. v3.1 skill fix: for a TEMPLATE_DELETED file the project keeps, pass it via `deleted_files` at finalize; remove the `deletedAcknowledged` stamp from 6a and 7b; state the auto-drop case. Related server work in progress: `fix/atomic-write-retry` (retry `os.replace` on PermissionError with backoff, remove own `.tmp` on final failure) — when it ships, the skill's live-run contract line pins that version as the minimum server version.
