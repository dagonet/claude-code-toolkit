...
>
> **Known defect:** `setup-project.{sh,ps1}` currently generate `<target>/.claude/.mcp.json`, which is therefore never loaded. For `dotnet`, `dotnet-maui`, and `rust-tauri` that file is the sole registration of `dotnet-tools` / `rust-tools` / `windows-mcp`, so those servers have not been active in generated projects. Existing projects should move the file to the repo root. `enableAllProjectMcpServers` does not help — it governs auto-approval, not file discovery.
