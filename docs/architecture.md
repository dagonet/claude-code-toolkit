...
| **PostToolUse** on `Bash\|PowerShell` | `hooks/bash-output-guard.sh` — a `tool_response.stdout` over 12,000 chars is written whole to `$TMPDIR/claude-bash-out/<session>-<epoch>.log` and replaced in the transcript by head 4,000 + a marker naming the log + tail 4,000, via `hookSpecificOutput.updatedToolOutput` (same shape as `tool_response`, sibling fields copied). Always exits 0, registered **unwrapped** — it can only ever pass output through | All |
