...
|------------|---------|-----------|
| **PreToolUse** on `Bash\|PowerShell` | The three git gates — `hooks/pre-commit-test.sh`, `hooks/no-push-main.sh`, `hooks/gate-before-merge.sh` — parse `tool_input.command`, split it on `&& \|\| ; \|` and newlines, unwrap `bash -c "…"`-style payloads, and match unanchored (fail-closed). `git -C <path>` retargets the repo; `<cwd>/.claude/git-guard-off` disables all three. Superseded the v2.0 blanket Bash-git block, which banned the git CLI outright and blocked 1,240 turns in 6 weeks (deleted in v2.1) | All |
