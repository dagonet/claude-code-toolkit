...
7. **For .NET / Rust-Tauri**: run `/build` to confirm the project-level `dotnet-tools` / `rust-tools` MCP is operational (generated by the setup script in `<project-root>/.mcp.json`; requires `--mcp-dev-servers-path` at setup time)
