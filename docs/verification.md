...
1. **Delivery, corrected in v4.0.1 (item 14, re-measured with this repo's own `zz-load-probe.md`).** A `.claude/rules/*.md` file carrying a `paths:` key is delivered only when a tool call touches a file that key matches, and is never present at session or subagent start — that much holds. But a rules file with **no `paths:` key** is not "delivered to nobody": it loads at **every session start**, at the same priority as `CLAUDE.md`, exactly like the PROJECT-CUSTOM region does. So the region is not the *only* always-on channel — `.claude/rules/project.md` (unscoped by design) is a second one. The reason always-on project conventions still belong in `CLAUDE.md`'s PROJECT-CUSTOM region rather than in `project.md` is not delivery — it is that writing the same rule in both places makes it exist twice and drift; `project.md`'s own seeded header says so. Safety rules, prohibitions and tool-selection guidance CAN technically live in an unscoped rules file; the toolkit's convention is to keep them in the region instead, for that single-source-of-truth reason.
