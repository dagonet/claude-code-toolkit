...
- Hard-blocks PR merges (and enabling auto-merge) unless a fresh gate artifact exists: since v4.0.1 (item 17), the artifact lives at `<common git dir>/gate/last-pass.<sha>.json` — inside `.git`, shared by every worktree of the repo (worktree-aware — developer agents self-merge from worktrees), never a per-checkout `.gate/` directory — and the file must be younger than `GC_GATE_TTL_S` (`hooks/lib/git-cmd.sh`; 3600s = 60 minutes). Accepted if either its `sha` equals the current HEAD, or its `tree` equals `HEAD^{tree}` (the commit-time `run-gate.sh` chain: the gate ran against the index just before the commit, so its `sha` is the parent commit but its `tree` is already the new commit's) — a scan of every `last-pass.*.json` in the directory, newest first, covers the tree match when no file is named for the exact sha. A legacy `<repo toplevel>/.gate/last-pass.json` is still read as a fallback, with a deprecation NOTE on stderr.
...
- Reads the Gate command from PROJECT_CONTEXT.md (tolerates `- ` list markers, the `**Gate Command**:` label style, and backtick-wrapped commands), runs it, and on success writes `<common git dir>/gate/last-pass.<sha>.json` (`{"sha","tree","branch","ts","status":"pass"}`, atomically via a `.tmp` file + `mv`) and prints `GATE PASS <sha>`. `tree` is `git write-tree` (the index tree) — recorded only when the working tree matches the index (`git diff --quiet`); otherwise `tree` is `""` and only the sha match applies. On failure it deletes that sha's own artifact and exits nonzero. On every successful run it also prunes `last-pass.*.json` files older than 24h from the shared directory. Refuses to run (exit 2) if `RUN_GATE_ACTIVE=1` is already set — guards against a `**Gate**` command that shells back into `run-gate.sh` itself.
...
- **Main-thread edits** are allowed only on the PO write surface (`docs/plans/`, `PROJECT_STATE.md`, `PROJECT_CONTEXT.md`, `.claude/`, `CLAUDE.md` (pre-v4 manifests only; under manifest v4 write `.claude/project-instructions.md` — `deny-claude-md-writes.sh` enforces it), `CLAUDE.local.md`, `AGENT_TEAM.md`, paths outside the repo). Everything else denies with a model-visible DELEGATE message naming the right agent. Handles `notebook_path` (NotebookEdit) and backslash Windows paths.
- **Main-thread Bash** denies build/test-runner commands (`npm test`, `npm run test|build|e2e|coverage`, `npx vitest|jest|playwright`, `pytest`, `cargo test|build|run`, `dotnet build|test|run`, `mvn`, `gradlew`, `go test`) AND `hooks/run-gate.sh` — the PO verifies via the `<common git dir>/gate/last-pass.<sha>.json` artifact, never by running the suite.
