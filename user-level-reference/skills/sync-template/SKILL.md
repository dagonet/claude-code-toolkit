...
# HOOK_NAME and CMD are named explicitly on each iteration below, never read
# as shell positional parameters -- a positional reference here would also be
# the token the Skill tool substitutes when this skill is invoked with an
# argument, corrupting the probe.
probe_hook() {
  printf '{"tool_name":"Bash","tool_input":{"command":"%s"},"cwd":"%s"}' "$CMD" "$R" \
    | bash "$H/$HOOK_NAME.sh" >/dev/null 2>&1
  echo "$HOOK_NAME [$CMD] exit=$?"
}
HOOK_NAME="no-push-main"
for CMD in "git push origin main" "ls -la" "true"; do probe_hook; done
```
...
#### 8a. Expected lines, per mode AND per manifest situation (v4.1.0 — `LINES` is now 30, not 24)
...
|---|---|---|---|---|
| **v4** | manifest already migrated | `{server_skew}` | `{}` | `{template_behind_head, encoding_drift, project_md_seed_current, legacy_gate_dir, once_notes_changed, project_md_scoped_consistent}` |
| **v3 legacy** | manifest v3, synced from a v4.0.x toolkit checkout (still region-bearing) — no window, behaves exactly as before this release | `{server_skew, claude_md_identical, import_line_present, instructions_file_present}` | `{}` | same six as above |
| **v3 window** | manifest v3, synced from a v4.1+ toolkit checkout — migrate first | `{server_skew, claude_md_identical, import_line_present, instructions_file_present}` | `{status_clean}` **alone** (R-K) | same six as above |
