...
4. For every referenced hook script PRESENT at the project root, NOT tracked in the manifest, AND NOT already applied by any earlier step of this sync (the two sets are disjoint): **this is the SAME register-or-apply route steps 4 and 5 use for a template-class file, because `hooks/*.sh` is template-class and template-class has no keep-mine under v3.** `template_apply_file(project_path=".", file_path="hooks/<name>.sh", source="skip")` is refused, measured verbatim: `source='skip' is refused under manifest v3 for template-class files: there is no keep-mine class -- fix the template or declare a key`. Do this instead:
   - Call `template_get_diff(project_path=".", file_path="hooks/<name>.sh", diff_type="three_way")` (or compare byte content directly) against the template's current version.
   - **Identical to the template** → register it, writing nothing: `template_finalize_sync(new_files=["hooks/<name>.sh"])`. This is the same call step 5 uses to register a present `once`-class file, applied here to a template-class one that happens to already match.
   - **Differs from the template** → there is no keep-mine to fall back to: `template_apply_file(project_path=".", file_path="hooks/<name>.sh", source="template")` overwrites it with the template's version. If the project's current content has anything worth keeping (a local edit to that hook), move it to a project-owned location BEFORE this call — `source="template"` here is destructive by design, the same as it is everywhere else a template-class file gets applied.
