...
**Never write the last two as `git checkout main && git pull --ff-only`.** The mover rule blocks a branch change chained with a gated pull, so the skill's own recovery sequence would self-block under the hook it ships. Two commands, two calls. The two lines are IN the block on purpose: the reader who needs them is the one who was surprised by a refusal on `main` and is primed to read it as a broken release, and that reader copies from the block, not from the prose under it.
