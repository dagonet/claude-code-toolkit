...
<!-- The `**Test (...)**` lines above are PROSE for humans: the hooks match `**Test**:` or
     `**Test Command**:` exactly, so none of them is read as THE Test field. This repo is
     deliberately Gate-only — pre-commit-test falls back to the Gate, which mints the
     artifact, so one gate run covers both the commit and the merge. -->
