...
Every literal a hook greps is pinned by `scripts/verify-template-consistency.sh` (**192 assertions**), so none of the cuts could silently break enforcement — including the exact Superpowers header, the `superpowers:` token the checks require, and (new in PR4) that every rules file is genuinely path-scoped and that the relocated developer preferences survive in the skill that now carries them.
