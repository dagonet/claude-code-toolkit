...
**v2.0-pr4 scoped, because the rule only applies to some files.** Every variant's *Code Style (MANDATORY)*, *Enforcement Notes*, and *Project Conventions* sections moved verbatim into `.claude/rules/*.md` with a `paths:` frontmatter glob list — a C# style rule now enters context when Claude opens a `.cs` file and not before. **Corrected in v3.1, on measurement:** this section used to claim that a rule *without* `paths:` "loads at launch at CLAUDE.md cost". It does not — **it is delivered to nobody.** A rules file arrives only attached to a tool call touching a file its `paths:` key matches, is absent from a subagent's context at spawn even when scoped, and with no `paths:` key never arrives at all. So anything that must be true *before* work begins — safety rules, prohibitions, which tool to reach for — belongs in `CLAUDE.md`, not in a rules file. The toolkit ships scoped rules only, but for the opposite reason to the one previously given. The duplicated Spawn-Prompt Binding Table (already in `AGENT_TEAM.md` and enforced by a hook) went with it.
