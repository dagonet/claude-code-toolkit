...
|---|---|
| **Progressive disclosure** | `AGENT_TEAM.md` is **20,442 B and is *not* read at session start** — the bootstrap loads it only when spawning a sprint, writing a spawn brief, or answering merge/escalation questions. (The *Context Budget* table's older v4.1.0/v4.1.1 cells used to read 20,467 B, a pre-existing 13 B discrepancy against those tags' own `wc -c` (20,454 B); **corrected 2026-09-26 (post-release)** — see `docs/architecture.md`.) `VERIFICATION_PLAYBOOK.md` and all **8 skills** load on trigger, not up front. Language conventions live in path-scoped `.claude/rules/*.md` files that arrive only when Claude touches a matching file (13 files, 16,653 B across all six variants). |
